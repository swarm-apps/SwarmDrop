...
- [`dev-notes/knowledge/storage-abstraction.md`](dev-notes/knowledge/storage-abstraction.md) — 把 sea-orm 从 core 摘出去：切割线在 `DatabaseConnection` 不在 `entity`、SendWrapper 免改 trait、耦合面量化。**已落地**：entity 的 sea-orm feature 解绑 → 端口抽到 transfer → SQL 实现搬进 storage-sql → Web 端 IndexedDB `SessionStore`（含「SendWrapper 要连 JsValue 构造一起裹」「Web 断点续传只有接收方向成立」等落地实测）
...
| 浏览器传输端（wasm） | `crates/web/`（WebNode API 面；`store.rs` 是 IndexedDB 写穿的 SessionStore，`idb.rs` 是其底层） |
