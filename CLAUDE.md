...
Current desktop release: **v0.23.0**（bootstrap 独立版本线，当前 `bootstrap-v0.8.0`；移动 `mobile-v0.23.0`；CLI `cli/swarmdrop-cli-v0.7.0`）。
...
- **Version management:** 两条版本线，各自有真源 + 跟随项，**同一条线内必须一起改**：
  ...
  - **CLI（tag `cli/swarmdrop-cli-v*`）**：真源 `crates/cli/Cargo.toml`，无跟随项。
    ...
    事后用 `gh release edit` 修的）。桌面已到 0.23.x 而 CLI 从 0.1.0 重起，撞号是必然。
