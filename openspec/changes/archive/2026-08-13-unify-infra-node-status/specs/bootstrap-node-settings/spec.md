...
**Reason**: `NetManager::ensure_relay_intent` / `remove_relay_intent` 是平台中立的 core API，Web 端已验证运行时增删可即时生效。「改了要重启」不是内核限制，而是桌面与移动缺少对应 IPC。保留该要求会迫使用户为一次配置修改断开全部连接（包括在途传输）。

**Migration**: 引导节点的增删改走 `ensure_infra_intent` / `remove_infra_intent`，由 `InfraSupervisor` 在 1 秒内起第一轮收敛，状态经 `InfraLink` 观测。重启横幅从页级降为行级，仅保留在真正需要重启的设置项（`provide_lan_helper`，它是 bind 期装配）下方。
