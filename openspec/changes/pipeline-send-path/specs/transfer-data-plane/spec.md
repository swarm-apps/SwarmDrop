...
**Reason**: 拆成两条：单条通道这条仍然成立（已 MODIFIED 保留），但「依赖传输层背压」
与「接收方读写分离避免死锁」两条已与实现分叉一整代——背压自 `Window` 停等流控引入后
由**应用层**闭合（浏览器的 `RTCDataChannel` 根本没有接收侧背压，指望传输层的设计在
Web 端会静默失效）；而「读写分离」正好与现行的「整流顺序读写、刻意不 split」相反，
后者是 wasm lost-wakeup 的修复结论。

**Migration**: 背压要求见 MODIFIED 的「数据帧基于 fetch_plan 连续推送」下的
「队列满时的背压」与「满窗停等」两个场景；单条通道与不得 split 见 MODIFIED 的
「数据面承载于单条数据通道」。
