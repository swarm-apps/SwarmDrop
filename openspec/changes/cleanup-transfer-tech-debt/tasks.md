...
- [x] 3.5 拆 `receiver.rs` god-module：`handle_block_data`（~115 行）拆成 `decrypt_and_validate`/`ensure_sink`/`persist_chunk`/`emit_chunk_progress` 四段 + 瘦编排；bitmap 纯函数已随 3.1 移 `checkpoint.rs`。**剩**：receiver writer 的 mpsc 桥简化为与 sender 对称的单 reader/writer 直写（结构性改 run_data_channel，单独小步做）
