...
- [x] 3.2 range 校验 dedup（**复核后决定不做**）：4 处（`receiver.rs` validate_fetch_plan/validate_block_range、`resume/validation.rs` validate_fetch_plan/validate_checkpoint、`sender.rs` write_range）的 length==0/empty-file/chunk 对齐规则**各有意义不同**，真正共享的只有「`offset+length` 溢出 + `end>file_size`」薄核心。**轮6 /simplify 的 reuse agent(R2) 独立复核确认同一结论**：抽全量薄核心会把 `receiver.rs::validate_fetch_plan` 的两条不同用户可见错误消息（「range 溢出」vs「超出文件大小」）并成一条 = 改行为，且这些是 base 已存在的 pre-existing 逻辑（非本次新引入重复）。价值有限，决定保留现状不抽取
