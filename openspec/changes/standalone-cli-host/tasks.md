...
- [x] 6.1 `pair` 生成：输出邀请链接 + 邀请标识，并提示在浏览器打开该链接可得二维码
...
- [ ] 10.11 **邀请清单与撤销入口** —— **已移交 `cli-command-surface`**（2026-08-19）。
      `CLAUDE.md` 要求三端都提供，CLI 是唯一没有的那个（`list_invites` / `revoke_invite`
      零调用）。在自动接受入站邀请之后它更重要：邀请泄露时，眼下唯一的作废手段是
      `swarmdrop stop`，而那**根本不管用**——邀请已落盘，重启回来它还在。
      移交的原因是它牵出了整个命令面：加 `swarmdrop pair list` 会与 `pair` 的位置参数歧义，
      于是命名、子命令层级、以及「哪些命令需要节点」都要一并定，超出本 change 的收尾范围
