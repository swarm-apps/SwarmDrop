...
MCP Server SHALL 新增一个 `get_inbox_file` Tool，用于在检索命中后定位单个已接收文件的本地路径，便于 Agent 进一步处理。该 Tool SHALL 接收定位参数（条目 id，以及条目内文件标识，如文件 id 或相对路径），返回该文件的 `local_path`、文件名、大小，以及 `missing` 标志。当目标文件在磁盘上已不存在（`missing = true` 或路径不可达）时，Tool SHALL 明确告知文件缺失而非返回无效路径。
