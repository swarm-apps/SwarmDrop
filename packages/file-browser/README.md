...
**动作还分两档：主动作常驻，其余悬停才露出**（`hasPrimaryAction`）。今天唯一的主动作是
`onDownload`——它是浏览器端把文件取回本机的**唯一**途径（那一端没有「打开文件」也没有
「在文件夹中显示」），压在 hover 后面等于整个入口不存在；桌面的动作全是便利项（文件早已
落盘），继续 hover。判据仍然是「这组动作里有没有主动作」，**不是「跑在哪一端」**——同一条
「不传就不渲染」的路子，包依旧不认识平台。

**集合级动作走 `FileBrowser` 的 `headerActions` 插槽**，不进 `FileBrowserActions`：后者每一项
都绑定在**树里的一个节点**上，「全部下载」在那套模型里没有位置。它渲染在表头、视图切换按钮
左边——「6 项 · 3.9 MB」那行说的正是这个集合。

**但「一个目录」是节点，不是集合。** `onRemove` 与 `onDownload` 因此都收
`FileBrowserTarget`（`{type:"file"} | {type:"directory"}`）而不是 `FileBrowserItem`。
配套的契约是：调用方**不得**把目录拆成「循环调 N 次文件动作」——目录取回必须能被单独
实现（为什么循环那条路在浏览器上不成立，见 `docs/app/app/_lib/zip-download.ts`）。

`pendingIds` 相应地容纳两种身份，两者都取自 **target 本身**：文件用 `item.id`、目录用
`relativePath`。要点是**没有一种身份来自建树时派生的节点 id**——那套编码是
`buildFileBrowserTree` 的私事，抄到调用方就会在 L1 改拼法时静默失配（spinner 再也不亮，
且不会有任何测试或类型报错）。

⚠️ 调用方手上若只有主键（收件箱只有 `fileId`），文件那半仍需用 **L1 的公开构造器**
（`inboxFileId` 等）换算成 `item.id`——那是复用编号规则，不是复制它。判据是：
用 `shared-view` 导出的函数算，不要手拼字符串。

⚠️ **目录动作只存在于树形视图。** 网格是扁平的文件卡片，没有目录这个节点——而收件箱的
默认视图恰恰是网格（`shared-view` 的 `view-preference.ts`）。所以目录级能力永远只能是
增强，不能是某件事的**唯一**入口；那件事的兜底必须放在两个视图都有的表头
（`headerActions`）。收件箱的「全部下载」就是这么摆的。
