...
需要 **Node 22+** · **pnpm 11+** · 较新的稳定版 **Rust**（1.85+）· **JDK 17** · **Android NDK r27c**（iOS 还需 **Xcode 16+**）。

```bash
# --recursive 不能省：Rust 桥以 path 依赖引用 libs/ 里的 swarm-p2p-core，
# 不拉 submodule 会直接编译失败。
git clone --recursive git@github.com:swarm-apps/SwarmDrop.git
cd SwarmDrop/mobile

# 移动端是独立的 pnpm workspace，必须在 mobile/ 下装
pnpm install
...
- **SwarmDrop** —— 设备间文件传输。[桌面端](https://github.com/swarm-apps/SwarmDrop) · [移动端](.)
