...
//! 经 `ContentResolver.openFileDescriptor` 拿到的 fd **不归本进程所有**——它指向
//! `/storage/emulated/0` 的 FUSE 挂载，真正的持有者在 DocumentsProvider 那一侧。
//! 长时间大文件写入期间它会失效，而 `FileChannel` 无从知晓：下一次 `lseek` 直接
//! `EBADF`，且 channel 自己仍然报告为 open。
//!
//! 2026-08-07 实测：接收 311 MB 单文件稳定在 45 MB 处崩掉，Web 与桌面两个发送端都能复现；
//! 把接收目录换成应用私有目录则完全正常。所以**随机写只施加于本进程完全拥有的 fd**，
//! 外部位置只在 publish 时被顺序写一次。
//!
//! ## 顺带消除的一跳
//!
