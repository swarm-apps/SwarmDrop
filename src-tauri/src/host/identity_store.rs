//! 桌面端设备身份与已配对设备列表的文件存储。
//!
//! 三个桌面平台统一走文件，**不依赖系统钥匙串 / 凭据管理器 / Secret Service**。
//!
//! ## 为什么不用 keychain
//!
...
//! ## 两条与 dev 实现的关键差异
//!
//! 1. **原子写**（[`write_json_atomic`]）——dev 用 `fs::write` 直接覆盖，写到一半断电就是
//!    一个截断的 JSON。私钥文件损坏不可恢复（身份丢失 = 所有配对失效）。
//! 2. **读取失败不降级**（[`read_json`]）——dev 解析失败返回默认值，于是 core 会「生成新
//!    身份并覆盖原文件」。一次磁盘坏块就静默换掉身份，用户只看到「设备列表空了」。
...
/// `identity.json` 是照这个结构手抄的第二份（那个 example 够不到本 crate 的私有类型）。
...
struct IdentityFile {
    /// protobuf-encoded Ed25519 keypair。`None` = 无身份，触发 core 生成新身份。
    ///
...
/// 桌面端 [`KeychainProvider`] + [`PairedDeviceStore`] 实现。
///
...
impl DesktopIdentityStore {
    pub fn new(app: &AppHandle) -> CoreResult<Self> {
        // **本机数据目录，不是漫游目录**：Windows 上 `app_data_dir` 是 `%APPDATA%`，
        // 会被域漫游配置文件同步到服务器——私钥不该跟着漫游。macOS 与 Linux 上两者
        // 解析到同一个目录，所以这不是平台分叉，只是在 Windows 上恰好落到了对的那个。
    ...
    /// 密钥文件的绝对路径。诊断 UI 要把它展示给用户——身份改为明文文件后，
    /// 「存在哪」是备份 / 迁移 / 收紧权限的前提。
    pub fn identity_path(&self) -> &Path {
        &self.identity_path
    }

    async fn read_identity(&self) -> CoreResult<IdentityFile> {
        Ok(read_json(&self.identity_path)
            .await
            .map_err(ReadJsonError::into_core)?
            .unwrap_or_default())
    }
    ...
    ///
    /// **锁必须罩住「读」到「写」的整段**，只锁写那一下是不够的——这个 crate 里曾经就是
    /// 那样，等于没锁：`save_identity` 与 `save_webrtc_certificate_pem` 并发时（UI 的
    /// `start` 撞上 MCP 的 `ensure_node_running`，两者都会进
    /// `load_or_create_webrtc_certificate`），两边各自读到同一份快照，后写的那份把先写的
    /// 字段覆盖掉。丢 `webrtcCertificatePem` 意味着 certhash 变、已发出的邀请地址全部失效；
    /// 丢 `keypair` 更糟——下次启动会静默生成新身份，所有配对作废。
    async fn update_identity(
        &self,
        mutate: impl FnOnce(&mut IdentityFile) + Send,
    ) -> CoreResult<()> {
        let _guard = identity_write_lock().lock().await;
        let mut file = self.read_identity().await?;
        mutate(&mut file);
    ...
    ///
    /// 被替换的 keychain 后端把 `NoEntry` 当成删除成功（`delete_entry_if_exists`），
    /// 这里保持同一语义：删除一个不存在的东西是 no-op，不该产生副作用，更不该为此付一次
    /// fsync。
    async fn clear_identity_field(
        ...
        let _guard = identity_write_lock().lock().await;
        let Some(mut file) = read_json::<IdentityFile>(&self.identity_path)
            .await
            .map_err(ReadJsonError::into_core)?
        else {
            return Ok(());
...
///
/// **进程级 static 而不是实例字段**：`host.rs` 的两个端口工厂各自 `new` 一个
/// `DesktopIdentityStore`，实例级锁根本罩不到彼此。跨进程不在此列——桌面有 single-instance
/// 插件把关。
///
/// 用 async 锁（`tokio::sync::Mutex`）是因为临界区里有 `.await`（读文件 + 写文件），
/// `std::sync::MutexGuard` 不能跨 await 存活。
///
/// 只保护 `identity.json`：`paired-devices.json` 这侧没有 RMW（`save_paired_devices` 是整份
/// 覆写），它的读-改-写在 core 的 `paired_devices` 里，那层的取舍见
/// `PairedDeviceStore` 的 trait 文档。
...
#[async_trait]
impl KeychainProvider for DesktopIdentityStore {
    ...
    async fn load_webrtc_certificate_pem(&self) -> CoreResult<Option<String>> {
        Ok(self.read_identity().await?.webrtc_certificate_pem)
    }
...
#[async_trait]
impl PairedDeviceStore for DesktopIdentityStore {
    /// **解析失败**降级为空列表，**I/O 失败**上抛。两者不能合并处理。
    ///
    /// - 解析失败降级：内容确定坏了，重试无用。而
    ///   `initialize_identity` 里那句 `load_paired_devices().await?` 会把 `Err` 变成
    ///   「`deviceId` 为 null → 节点永远起不来」，且诊断里给的是**身份文件**路径、不是这份，
    ///   用户连该删哪个文件都不知道。一条坏记录不该锁死整个应用——大不了重新配对。
    ...
    async fn load_paired_devices(&self) -> CoreResult<Vec<PairedDeviceInfo>> {
        match read_json::<Vec<PairedDeviceInfo>>(&self.paired_devices_path).await {
            ...
            Err(err @ ReadJsonError::Io(_)) => Err(err.into_core()),
        }
    }
...
/// [`DesktopIdentityStore::load_paired_devices`] 的文档。
#[derive(Debug)]
enum ReadJsonError {
...
impl ReadJsonError {
    fn into_core(self) -> CoreError {
        match self {
            Self::Io(message) | Self::Parse(message) => CoreError::Identity(message),
        }
    }
}
...
///
/// **绝不降级为默认值**——那正是 dev 实现的做法：解析失败返回 `Default`，于是
/// `load_identity()` 返回 `Ok(None)`，core 走「生成新身份」的路径**并覆盖原文件**。
/// 一次磁盘坏块、一次被外部工具改坏，就把身份不可恢复地换掉了，而用户看到的只是
/// 「设备列表空了」，没有任何错误提示。降不降级由调用方按数据的可恢复性决定。
async fn read_json<T: DeserializeOwned>(path: &Path) -> Result<Option<T>, ReadJsonError> {
...
///
/// 四条安全属性交给 [`tempfile`] 而不是自己搓（它已经在依赖树里，见 `Cargo.toml`）：
/// 随机文件名、`O_EXCL` 创建、unix `0o600`、以及**任何提前返回都在 drop 时清掉残留**。
/// 自己写这四条要 55 行，其中一半篇幅是在向读者证明标准库的 `open(2)` 语义——
/// 比如「`mode` 只在真正创建文件时生效，所以复用一个残留 tmp 会让私钥继承它的权限」。
/// 那份证明这个生态已经做过一次了。
///
...
async fn write_json_atomic<T: Serialize + ?Sized>(
    ...
    let text = serde_json::to_string_pretty(value).map_err(CoreError::Serialization)?;
...
fn write_text_atomic_blocking(path: &Path, text: &str, durability: Durability) -> CoreResult<()> {
    use std::io::Write;

    let parent = path
        .parent()
        .ok_or_else(|| CoreError::Identity(format!("{} 没有父目录", path.display())))?;
    std::fs::create_dir_all(parent)
        .map_err(|e| CoreError::Identity(format!("创建 {} 失败: {e}", parent.display())))?;
    ...
    let mut tmp = tempfile::Builder::new()
        .suffix(".tmp")
        .tempfile_in(parent)
        .map_err(|e| CoreError::Identity(format!("创建临时文件失败: {e}")))?;

    tmp.write_all(text.as_bytes())
        .map_err(|e| CoreError::Identity(format!("写入 {} 失败: {e}", path.display())))?;
    if matches!(durability, Durability::Fsync) {
        tmp.as_file()
            .sync_all()
            .map_err(|e| CoreError::Identity(format!("落盘 {} 失败: {e}", path.display())))?;
    }

    tmp.persist(path)
        .map_err(|e| CoreError::Identity(format!("替换 {} 失败: {e}", path.display())))?;
    Ok(())
}
...
mod tests {
    ...
    /// 会在这一行之后立刻被删掉。此前这里返回的是裸 `PathBuf` + 「跑之前先 remove_dir_all」，
    /// 于是每跑一次 `cargo test` 就在 `/tmp` 留下一个 0600 的 identity.json 长期不清。
    fn temp_store(tag: &str) -> (tempfile::TempDir, DesktopIdentityStore) {
        let dir = tempfile::Builder::new()
            .prefix(&format!("swarmdrop_identity_{tag}_"))
            .tempdir()
            .expect("tempdir");
        let store = DesktopIdentityStore {
            identity_path: dir.path().join(IDENTITY_FILE),
            paired_devices_path: dir.path().join(PAIRED_DEVICES_FILE),
        };
        (dir, store)
    }
    ...
    ///
    /// 降级为 `Ok(None)` 会让 core 生成新身份并覆盖原文件——一次可恢复的读取故障
    ...
    async fn saving_paired_devices_does_not_touch_identity_file() {
        ...
        store
            ...
            .await
            .unwrap();
        let before = tokio::fs::read(dir.path().join(IDENTITY_FILE))
            .await
            .unwrap();

        store.save_paired_devices(&[]).await.unwrap();

        let after = tokio::fs::read(dir.path().join(IDENTITY_FILE))
            .await
            .unwrap();
    ...
    /// `initialize_identity` 里那句 `load_paired_devices().await?` 会把这里的任何 `Err`
    /// 变成「`deviceId` 为 null → 节点永远起不来」。
    #[tokio::test]
    async fn corrupt_paired_devices_degrades_to_empty() {
        let (dir, store) = temp_store("paired-corrupt");
        tokio::fs::write(dir.path().join(PAIRED_DEVICES_FILE), b"{ not an array")
            .await
            .unwrap();
