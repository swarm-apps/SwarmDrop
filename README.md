...
<h3>📱</h3>
<b>全平台</b><br>
<sub>Windows · macOS · Linux<br>iOS / Android（<a href="https://github.com/swarm-apps/SwarmDrop-RN">SwarmDrop-RN</a>）</sub>
</td>
</tr>
</table>
...
> 移动端（iOS / Android）已迁移到独立的 [SwarmDrop-RN](https://github.com/swarm-apps/SwarmDrop-RN) 项目，构建与发布在那边进行。
...
| 桌面端自动更新 | ✅ |
| 多语言支持 (zh · en · zh-TW · ja · ko · es · fr · de) | ✅ |
| 移动端（iOS / Android · 独立仓库 [SwarmDrop-RN](https://github.com/swarm-apps/SwarmDrop-RN)） | 🚧 进行中 |
| 自定义引导节点 | ✅ |
| 断点续传 | 📋 计划中 |
| MCP 集成 (AI 助手发文件) | 📋 计划中 |
...
│   ├── stores/             #   Zustand 状态管理
│   └── locales/            #   国际化翻译文件
├── src-tauri/              # 桌面端 Tauri 壳（业务逻辑由 crates/core 提供）
│   └── src/
│       ├── commands/       #   Tauri IPC 命令薄壳
│       ├── network/        #   NetManager 类型别名 + 事件循环
│       ├── transfer/       #   文件传输实现（offer / sender / receiver / crypto）
│       ├── file_source/    #   桌面文件读取
│       ├── file_sink/      #   桌面文件写入
│       ├── host/           #   桌面 adapter（keychain / notifier / updater）
│       └── mcp/            #   桌面 MCP server
├── crates/                 # 双端共享 Rust crate
│   ├── core/               #   网络 / 配对 / 设备 / 传输 / 协议核心
│   ├── entity/             #   SeaORM 实体定义
│   └── migration/          #   SeaORM 迁移
...
> 移动端开发请前往 [SwarmDrop-RN](https://github.com/swarm-apps/SwarmDrop-RN) 仓库。
