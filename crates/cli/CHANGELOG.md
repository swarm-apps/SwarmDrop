# Changelog

`swarmdrop` 命令行工具的变更记录。

> 这条版本线独立于桌面端（tag `v*`）与移动端（tag `mobile-v*`），tag 形如
> `cli/swarmdrop-cli-v0.1.1`（0.1.0 用的是旧形式 `cli/v0.1.0`，见 dist-workspace.toml）。
> 仓库根目录的 `CHANGELOG.md` 记的是桌面端，与本文件无关。

## [0.9.0] - 2026-08-22

### 新增

- `swarmdrop invite qr <邀请> [--size <px>]`：把一张邀请渲染成二维码。SVG 写 stdout，
  `--json` 时包成一行 `{"svg": …}`。纯计算——**不启动节点、不走本机通道、不读任何记录**，
  因此任何时刻都能回答。

  它补的是一个此前没有出口的缺口：桌面端与落地页各自直接调
  `swarmdrop_invite::invite_qr_svg`，移动端拿模块矩阵自绘，而**隔着命令行说话的调用方
  （agent harness 的插件）没有任何路径拿到同一张码**——只能自己在浏览器里再实现一遍编码，
  那正是 `crates/invite/src/qr.rs` 点名要避免的漂移。本命令什么都不改，只是把那个单点
  暴露出来。

  `--size` 是二维码**本体**的边长（不含渲染端套的白卡内边距），同时是**地址预算**：
  码面放不下时按价值反序回收邀请里的可拨地址提示，所以传小了不会失败、只会让这张码里的
  可拨路径变少（链接本身不受影响）。默认 240，与三端图形界面一致。

  **终端里仍然不画码**：那条决定拒绝的是把码画成字符块（69 列 × 35 行塞不进 80×24，
  换象限块又会被字符宽高比压成 2:1 而读不出来），与把一段 SVG 交给一个会把它画进像素的
  程序是两回事。

## [0.8.0] - 2026-08-22

### 新增

- `swarmdrop status` 的输出增加 `daemonVersion`：应答这条请求的常驻节点自己的版本。
  载荷是追加字段，旧客户端不受影响。

### 修复

- 常驻节点与命令不是同一个版本时，`swarmdrop status` 会说出两个版本号并提示重启节点。
  此前只有「常驻节点认不得这条请求」时才有提示，而那要等到真的发出一条它不认识的命令。
- 客户端解析响应失败时只报 serde 原文。现在指出最可能的成因（常驻节点比这条命令新，
  即本机装了两份），与服务端那侧的措辞对称。

## [0.7.1] - 2026-08-22

### 修复

- `swarmdrop inbox search` 的标题未经消毒直接输出。文本条目的标题是对端正文的前
  160 字节，带 ANSI 转义的正文会在检索结果里执行并改写屏幕；列表与选择菜单一直是
  消毒过的，只有检索结果不是。
- `swarmdrop inbox search` 给片段多加了一层省略号。内核只在真截断的一侧贴，渲染层
  再包一层会把「片段就是全文」印成「前后还有内容」。
- `swarmdrop transfer show` 的进度停在上次终结时的值，且从不带速率。它现在与
  `transfer list` 一样，由常驻节点盖上实时进度与 `speed` / `eta`。

## [0.7.0] - 2026-08-22

### 新增

- `swarmdrop inbox search <关键词>`：子串检索收件箱，覆盖标题、来源设备名、文本正文与
  文件名；支持 `--limit` 与 `--include-archived`。
- `swarmdrop watch` 的 `transferProgress` 事件增加 `speed`（B/s）与 `eta`（秒）。
  新增字段不递增 `SCHEMA_VERSION`。

### 修复

- `swarmdrop send` 与 `swarmdrop transfer watch` 的速率显示偏高一个数量级。两处改用
  核心给出的 `speed` / `eta`（3 秒滑窗），不再由 indicatif 从进度条位置估算；停滞与
  算不出来时显示占位符而非 `0 B/s`。

## [0.6.0] - 2026-08-21

### 新增

- `swarmdrop config`：读写本机设置。子命令 `list` / `get <键>` / `set <键> <值>` /
  `unset <键>`；可配项为封闭集合 `device-name`、`receive-dir`。
- `swarmdrop bootstrap`：管理引导 / 中继节点。子命令 `list` / `add <地址>` /
  `remove <地址>`，地址接受唯一前缀，缺省时进入交互选择。
- 接收落点支持持久化配置。优先级为 `SWARMDROP_RECEIVE_DIR` → 持久化配置 → 内置默认。
- `config` 的 `--json` 读面按项给出 `value` / `source` / `configured` / `overriddenBy`，
  写入结果给出生效状态（`applied` / `pendingStart` / `overridden`）。
- 引导节点清单由编译期常量改为「内置清单 + 用户增删」。持久化 `custom` 与 `removed`
  两个集合，未做过增删的用户随版本更新获得新的内置地址。
- 添加引导节点前执行同步校验：Multiaddr 可解析、含 `/p2p/`、传输为本端点所装配、
  与既有条目不重复；全程无网络往返。
- `swarmdrop status` 增加引导节点一行（总数 / 已连接数 / 中继就绪数）。

### 变更

- 常驻节点运行期间，改设备名与增删引导节点即时生效，无需重启节点。改设备名复用
  `swarmdrop-core` 的改名编排，同步更新 identify 的 `agent_version`。
- 常驻节点无法解析请求时的提示改为指向版本错配（升级 CLI 不会重启常驻节点），
  并给出 `swarmdrop stop` / `swarmdrop start` 的处置。
- 配置读写不启动节点；存在常驻节点时经本地通道复用它，不另起临时节点。

### 修复

- `swarmdrop watch` 不再将离线的已配对设备从设备表中移除。此前订阅面直接转发内核
  `DevicesChanged` 事件的载荷，而该载荷是本次运行被观测到的 peer 集合，与订阅面契约的
  「全量已配对设备」不同口径；现设备表仅由「向节点现取已配对名册」这一处产出。
- 丢弃判据覆盖全部产帧路径：触发设备表现取的内核事件被丢弃时同样上报截断。

## [0.5.0] - 2026-08-21

### 新增

- **`swarmdrop invite create --json --decide-from-stdin`：把配对的确认交给调用方，
  而不是终端前的人。** 供托管本命令的图形前端使用——本仓的 DeepSeek Harness 插件
  （`dsh-swarmdrop`）就是第一个消费者：它在浏览器面板里画出对端信息，由用户点「接受」。

  线格式是这条命令**既有的** NDJSON 事件流（`inviteCreated` / `pairingRequest` /
  `pairingDeclined` / `pairingRequestExpired` / `paired`），新增的只是反方向：调用方往
  stdin 写一行 `{"pendingId":N,"accept":true}`。

  **它不是 `--auto-accept` 的别名，方向正相反。** 那条谁都放行、无人核对身份；这条把
  对端的节点标识交给一个会展示给人看的程序，人还在环里，只是站的位置变了。因此它
  **fail-closed**：stdin 关闭 = 「问的那个人走了」，手上那条顺手拒掉并退出。

  两种「不算答复」的输入被**跳过而不是当作拒绝**——答的是别的 `pendingId`（对一条已
  失效请求的迟到回复），或者压根不是合法的决策对象（一行垃圾）。把它们读成拒绝会让
  用户在界面上看到「已拒绝」并去排查自己那台设备，而实际什么也没发生。

  `--json` 是硬要求（`requires`），与 `--auto-accept` 互斥（`conflicts_with`）。
  「配对窗口只在本命令运行期间打开」这条不变量原样保留：进程退出，窗口就关。

### 修复

- **`invite create --json` 不再在宿主关掉 stdout 时 panic。** 那五处 JSON 输出用的是
  `println!`——Rust 启动时把 `SIGPIPE` 设成忽略，于是往一条已关闭的管道写会让 `write`
  返回 `EPIPE`，而 `println!` 对此的反应是 panic（退出码 101）。宿主结束一次配对时先关
  读端是常规动作，而 101 会被读成「它崩了」。

  判据与 `watch --json` 同源，此前只有那条流用上：两条路径现在共用
  `render::emit_line`。`--decide-from-stdin` 让这条更容易撞上——它把 `invite create`
  从「打两行就完」变成了一条由宿主持有的长驻流。

### 变更

- `invite create --json` 的 `inviteCreated` 帧新增 `event` 字段。**向后兼容**：
  旧调用方读的 `invite` / `id` 原样保留。加它是因为这条命令现在是一条 NDJSON 流，
  而流上每一帧都要能只靠自己认出是什么。

## [0.4.0] - 2026-08-21

### 新增

- **`swarmdrop mcp`：以 MCP server 形态运行，让 agent 经标准输入输出调用 SwarmDrop。**
  面向 agent harness（DeepSeek Harness、Claude Code、Codex …）——它们各自的扩展机制互不
  相同，唯一的公约数是「能执行一条命令」。14 个工具：发文件 / 发文本、列设备、收件箱的
  列检取路径、传输的列查与暂停恢复取消、网络状态。

  **stdout 只承载协议帧**，日志与诊断一律走 stderr；交互提示被强制关闭（提示会吃掉一帧
  协议消息，而终端里手动调试时 TTY 判据恰好拦不住它）。节点按「有常驻就复用、没有就自持」
  接入，且**持有到 server 退出**——每次工具调用现起现关的话，每一次发送都要重连引导节点
  并做 NAT 探测。

  ⚠️ **它不打开配对窗口**：入站配对一律被拒且不消费邀请凭证。窗口只应在人正在等待时打开
  （`swarmdrop invite create` 运行期间），而一个长驻的 MCP server 后面没有人在看。

- **`swarmdrop watch`：订阅本机发生的事件，推给调用它的程序。** 覆盖收件箱、传输、设备
  三类变化；`--json` 下是 NDJSON（每行一条完整事件），否则是面向人的事件行。

  订阅建立时先给一条**基线**（最近 N 条收件箱 + 已配对设备及在线状态 + 未完成传输，
  `--inbox-limit` 可调），其后只推增量。每条事件带 **schema 版本**与**订阅内单调序号**：
  消费方会把这些事件持久化并回放，仅凭事件自身就要能判定格式与漏读。

  **不启动节点**，也不因为没有节点而失败：没有常驻节点时照样给基线并等它出现，节点关停后
  推一条 `nodeUnavailable` 继续等。调用方因此不必关心自己与节点的启动顺序。

  进度是**聚合**的（无逐文件明细）并降频到约 1 秒一帧；读得太慢导致边沿事件被丢弃时会推
  一条显式的 `truncated`，**绝不静默**——但慢消费方绝不会拖慢正在进行的传输。

  ⚠️ 与 `swarmdrop transfer watch` 不是一件事：那条是给人看的传输面板（重绘全量快照、
  带热键、画在 stderr），这条是给程序用的增量推送（写在 stdout）。

### 修复

- **`SIGTERM` 现在与 `SIGINT` 同等对待，且以成功退出。** 此前只接 `SIGINT`，而长驻命令的
  主要调用方（agent harness 的 `terminate()`、`systemctl stop`）发的都是 `SIGTERM`——
  于是**最常见的正常收摊路径**走不到清理、退出码非零，而脚本把非零读作失败并触发重启或告警。
  覆盖 `watch` / `mcp` / 前台 `start`。

  ⚠️ `mcp` 那条另有一层：MCP 的 stdio 传输把 stdin 交给了阻塞读任务，而被信号叫停时宿主
  仍握着 stdin——只是 `return` 的话运行时析构会等那次永不返回的读，**进程挂死**（比退
  非零更糟：服务管理器一直等到自己的超时，harness 留下一个僵尸子进程）。清理做完后直接退。

- **`swarmdrop mcp` 自持节点时会正确地摆成常驻形态。** 此前它持着单实例锁数小时却不建
  本地通道、不起被动接收：同机的每一条命令都撞「另一个进程正在启动，等待超时」，
  `swarmdrop watch` 以为本机没有节点，而那个节点在线、可达、已配对却**一个文件都收不下**。
  三种失败没有一种会报错。

- **停止信号不再可能被静默吞掉。** 几处 `select!` 循环此前每转一圈就丢弃并重建一次信号
  监听器，而 tokio 的接收端看不到自己出生前的那次通知、OS 处理器又不再兜底——落在缝隙里
  的 `SIGTERM` 就此消失（表现是「`systemctl stop` 没反应，等满超时被 SIGKILL」）。

- **`swarmdrop watch` 的读端被关掉时正常收摊，不再 panic。** `println!` 遇到 `EPIPE` 会
  panic 退 101，而宿主结束订阅时先关读端是常规动作，101 会被读成崩溃并触发自动重启。

- **订阅重连有退避了。** 新客户端撞上没重启的旧常驻节点时（它不认识 `subscribe` 动词），
  此前是一个满速重连的死循环：烧满一个核、往宿主 stdout 灌事件、并在常驻节点里堆订阅者。

- **`list_inbox` 工具的 `include_archived` 真的生效了。** 此前它在宿主层按一个不存在的
  字段名过滤，谓词恒真——开关静默无效。

- **收件箱的变化现在由领域事件驱动。** 此前命令行端没有任何「收件箱变了」的信号可用；
  新增的三条领域事件（新增 / 归档 / 删除）让订阅面不必再从传输完成事件反推——那种反推
  依赖一条只以注释存在的顺序不变量，且对文本条目根本不成立。

## [0.3.0] - 2026-08-20

### 新增

- **`swarmdrop send --text`：发一段文本，不只是文件。** 对端在**收件箱**里收到它。
  正文三条来源，按标准输入是不是终端分流：命令行（`--text "内容"`）、管道（只给
  `--text` 时读到 EOF，`pbpaste | swarmdrop send --text --to laptop`）、`$EDITOR`
  （终端里只给 `--text` 时拉起，为多行正文准备——上限 64 KiB，而单行输入框连回车都收不下）。

  与位置参数互斥，一次只送一样东西。结尾的换行会被削掉，缩进与内部空行原样保留。
  成功时说的是**「已送达」**而不是「已发送」——它一直等到对端确实把正文存下来才返回。
  失败区分「未能送达对端」（退出码 4，可稍后重发）与「对端收到但没保存」（退出码 5）。

- **常驻节点自动接收入站文本。** 判据与文件一致：**已配对**即接收。少了这一支，
  新配对设备的默认信任档位（带「需要人工确认」）下**一条文本都收不到**——发送端会阻塞
  满五分钟确认窗口，然后拿到一句「对端未在确认窗口内接收」，看起来完全像网络问题。

- **`inbox show` / `transfer show` 给出内容在本机的位置。** 命令行端没有「在文件管理器中
  显示」那个按钮，路径本身就是答案；而接收落点可被 `SWARMDROP_RECEIVE_DIR` 改掉、
  只在节点启动那一刻打印过一次。单文件条目给文件自身的完整路径，多文件条目给容器目录；
  `transfer show` 只对接收方向给（发送方向没有本地落点）。

- **`swarmdrop update`：就地更新到最新版本。** 不带参数直接装；`--check` 只查不装。
  `--json` 下给出 `status` 字段（`installed` / `upToDate` / `updateAvailable` /
  `managedExternally`）供脚本判断——四种结局都带，包括「装成功了」那一格。

  **只有安装脚本（shell / powershell）装的那份会就地更新。** 用 Homebrew 或 npm 装的会
  被认出来，并转交给对应的包管理器（`brew upgrade swarmdrop` / `npm install -g
  swarmdrop@latest`）而不是硬更新——不去和它们争「哪个版本是当前版本」。认不出来的
  （`cargo install`、从源码构建、手动解压）只给一句通用指引和 releases 页链接，不猜。

  判据是 install receipt **且它指向正在运行的这个可执行文件**：只看「receipt 在不在」
  会把「先用脚本装过、后来改用 brew」的机器判错，于是自更新去改写另一个位置的文件，
  而用户实际执行的那份纹丝不动。

  **节点在跑时拒绝更新**（退出码 7），先 `swarmdrop stop`。Windows 根本不让覆盖运行中的
  可执行文件，而 Unix 让覆盖——那反倒更隐蔽：命令报「已更新」，还在跑的节点却仍是旧代码。

- **`swarmdrop start` 顺带提示有没有新版本**，一天最多查一次，提示走 stderr。
  `SWARMDROP_NO_UPDATE_CHECK=1` 关掉它；`--json` 模式下从不检查。

### 修复

- **`send --to <名字>` 撞上重名时，会列出候选的节点标识。** 此前只说「匹配到 2 台设备，
  请改用节点标识指定」——用户知道有歧义，却没有任何办法查出是哪两台。`device forget`
  那条路径一直是列出来的；两处措辞不同是因为它们各有一份实现，现在共用同一份。

- **没有常驻节点时 `send --to <设备>` 不再报「找不到已配对设备」。** 这条是既有缺陷：
  目标解析用的是「本次运行**发现**的对端」而不是「已配对设备表」，而一次性命令每次都新起
  临时节点，那张发现表在发出请求的一刻基本是空的。表现是 `device list` 明明列着那台设备、
  `send --to` 却说找不到。文件与文本共用这条解析，两支一起修好。

- **收件箱标题压成恰好一行。** 文本条目的标题是正文前 160 字节、**带着正文里的换行**，
  而列表、选择菜单、详情三处都假定它占一行——菜单尤其糟，dialoguer 按项计数、按行绘制，
  多出来的行会让选中高亮错位。同时滤掉控制字符：那是本程序第一次把**对端可控的字符串**
  画进终端菜单，一条含 `ESC[2J` 的文本会在菜单重绘时清屏。

- **`send` 在准备阶段不再一片空白。** 校验和与 bao 验签树是一次长阻塞计算——几个 GB 的
  文件要算几十秒，而这段时间屏幕上既没有进度也没有报错，看起来就是卡住了。根因是事件
  订阅建在 `prepare` **之后**，那串进度事件早已发完、无处可订。现在准备阶段有自己的
  进度条，且**与传输进度视觉可区分**（动词分别是「准备中」「传输中」）——两段共用同一个
  视觉原语正是那次「把准备读成传输、进而把新会话读成续传从 0% 重来」的原因。

- **有常驻节点在跑时，`send` 的进度直接显示在当前终端。** 那条路径上准备与传输的事件都在
  常驻节点那个进程里，客户端这边从敲下回车到传完一个字都没有。现在本地通道从严格的
  一问一答放宽成「一问、若干条进度、一答」：常驻节点把进度推过来，**由客户端用同一组
  进度条画出来**——两条路径（自持节点 / 常驻节点）因此看起来完全一样，不必另开终端敲
  `transfer watch`。

  不认识进度帧的命令不受影响：`request` 会安静地跳过它们，所以给某个动词加进度不会噎住
  任何一个既有命令。

### 变更

- 新增退出码 **7 = 更新失败**，与「节点不可用」（3）分开——脚本不该把「装不上新版本」
  当成「节点起不来」而去重启节点。

## [0.2.0] - 2026-08-20

### 新增

- **`swarmdrop transfer watch`：实时盯着正在进行的传输。** 每条未结束的会话一行进度条，
  正在传的那条显示速率与剩余时间，等待确认 / 已暂停的只显示状态（给它们画速率是在报告
  一个假事实）。面板上带热键：`p` 暂停、`r` 恢复、`c` 取消、`q` 退出——按下去弹的就是
  下面三条命令那套多选菜单。

  没有常驻节点时它照样能开：列出的是等着续传的那几条。`--json` 下退化成每秒一行的
  NDJSON 快照流，供脚本消费。

- **`swarmdrop transfer pause / resume / cancel`。** 不带标识时列出**此刻能做这个动作**的
  会话让你勾选（可多选）：暂停只列正在传的，恢复只列可续传的，取消只列尚未结束的——
  菜单里出现的每一条都当真做得成。

  三条都要求常驻节点在跑，且**绝不为它们起临时节点**：它们作用的是常驻节点内存里的活
  传输，临时节点里空空如也。没有节点时立即报错并指出先 `swarmdrop start`。

### 修复

- **传输列表与详情不再打印生的英文状态**（`active` / `suspended`），改为与桌面端同一套
  中文措辞（「传输中」「已暂停」「对方暂停」「已中断」「未及时处理」…）。

### 变更

- **`invite create` 不再在终端画二维码**，改为只输出邀请链接并提示「在浏览器打开它，
  页面会显示二维码」——链接指向的 `/p/` 落地页按需渲染。**`--no-qr` 开关随之删除。**

  判据是尺寸且没有可调空间：邀请的体积由签名与公钥主导而非地址，裁到 `fit` 的下界
  （只剩一条地址）仍是 69 模块；半块字符已经是长宽比正确的最密形态，于是终端占用恒为
  **69 列 × 35 行**起，标准 80×24 终端一屏半装不下，而命令还要在码下面继续输出。
  换成 2×2 象限块能把宽度减半，但会把码压成 2:1 的竖长条，扫码器直接读不出。

  扫码这条路径没有断，只是换了承载：浏览器没有终端的宽高比与行数约束。

### 修复

- **Windows 上 `swarmdrop start` 直接起不来**（报「本地通道路径不可用: not a named pipe
  path」）。本地通道此前一律取数据目录下的 `swarmdrop.sock`，而 Windows 的命名管道不在
  文件系统里——`interprocess` 只接受 `\\.\pipe\` 开头的名字。现在按平台分叉：Unix 仍是
  数据目录下的域套接字（因而继续受目录 0700 保护），Windows 用一个从数据目录派生的管道名
  `\\.\pipe\swarmdrop-cli-<hash>`，`--data-dir` 的多个实例互不串台。
  ⚠️ 已知缺口：Windows 的通道不在数据目录里，那道 0700 保护不到它，而管道默认 DACL 的
  实际放行范围本仓尚未实测。

### 改进

- **所有需要「目标」的命令都能不带参数运行**，缺的那个由交互补出来：

  | 命令 | 不带参数时 |
  |---|---|
  | `send` | 先列出已配对设备让你选，再逐行问要发什么（可拖进终端 · Tab 补全） |
  | `invite use` | 问你要邀请链接 |
  | `invite revoke` | 列出未过期邀请，**勾选若干张** |
  | `device forget` | 列出已配对设备，**勾选若干台** |
  | `inbox show` / `export` | 列出收件箱让你选；`export` 还会问目标目录（默认当前目录） |
  | `transfer show` | 列出传输记录让你选 |

- **撤销邀请与解除配对支持一次多个**，参数侧同样收多个（`invite revoke a b c`）——
  此前只能一次一个，脚本得把同一条命令循环敲 N 遍。
- **交互提示改用彩色主题**，长列表自动翻页。
- 发送的路径输入认得 shell 转义与引号，所以把几个文件一起**拖进终端**就能用；
  `~` 会展开。

  这些行为在**不可交互的环境里一个都不变**：`--json` / `--no-input` / 管道下缺参数仍然
  立即以用法错误退出（退出码 2），绝不停下来等一个不会到来的回答。

### 破坏性变更（`--json`）

两条命令的结构化输出因为「一次可处理多个」而改了形状。**只影响解析 `--json` 的脚本**，
人类可读输出与退出码不变：

| 命令 | 0.1.0 | 现在 |
|---|---|---|
| `invite revoke <id>` | `{"event":"inviteRevoked","id":…,"persisted":…}` | `{"event":"invitesRevoked","ids":[…],"revoked":N,"persisted":…}` |
| `invite revoke --all` | `{"event":"invitesRevoked",…}` | `{"event":"allInvitesRevoked",…}` |
| `device forget <dev>` | `{"event":"deviceForgotten","peerId":…,"name":…,"remaining":…}` | `{"event":"devicesForgotten","devices":[{"peerId":…,"name":…}],"remaining":…}` |

⚠️ `invite revoke --all` 的 `event` 一并改名，是因为逐张撤销现在占用了 `invitesRevoked`
——两者是不同的动作（撤指定的几张 vs 连这一瞬新签发的也作废），共用一个名字会逼脚本
去探测 `ids` 字段在不在才能分辨。

## [0.1.0] - 2026-08-19

首个正式版本。

### 是什么

SwarmDrop 的命令行宿主——桌面 / 移动 / Web 之外的第四个宿主，与前三者共用同一套 P2P 内核
与配对协议。无账号、无中心服务器，设备间端到端加密传输，局域网与跨网都走同一条路径。

### 命令面

```
swarmdrop start / stop / status      常驻节点的生命周期
swarmdrop send <FILES> --to <设备>   发送文件或目录
swarmdrop invite create | use | list | revoke
swarmdrop device  list | forget
swarmdrop inbox   list | show | export
swarmdrop transfer list | show
```

### 为脚本与 agent 设计

- **退出码区分失败原因**，无需解析文本：用法错误 2、节点不可用 3、对端不可达 4、
  传输失败 5、配对被拒 6、被中止 130。同一件事在「有常驻节点」与「无常驻节点」两条路径上
  给出同一个退出码。
- **`--json`** 让每条命令输出结构化结果，且 stdout 只有最终结果——进度、诊断、日志一律走
  stderr。
- **`--no-input`** 显式关闭交互，用于服务单元与 CI；缺参数时不会挂起等待，而是以用法错误退出。
- **只读命令不启动节点**：`device list` / `invite list` / `inbox list` / `transfer list` 等
  直接读本机记录，毫秒级返回。邀请泄露后的撤销尤其不该要求先启动一个可能失败的节点。

### 交互补全

参数给全就直接执行；缺参数且有终端时列出候选让你选（撤销哪张邀请、解除哪台设备）；
缺参数又没有终端则立即以用法错误退出——不猜默认值，因为这两个操作都不可逆。

### 与图形界面并存

命令行宿主使用**独立的设备身份**与数据目录，同一台机器上与桌面端同时运行时互不干扰，
在对方的设备列表里显示为两台设备（名字带 ` (cli)` 后缀）。

### 安装

```bash
# macOS / Linux
curl -fsSL https://github.com/swarm-apps/SwarmDrop/releases/download/cli/v0.1.0/swarmdrop-cli-installer.sh | sh

# Homebrew
brew install swarm-apps/tap/swarmdrop

# npm（给 agent harness 用：按平台解析到对应二进制）
npx swarmdrop --help
```

支持 macOS（Apple Silicon / Intel）、Linux（x86_64 / aarch64，gnu 与 musl）、Windows x86_64。
