...
/// 输出一张邀请的二维码。
///
/// ⚠️ **这不是上面那条「终端里不画二维码」的反悔，两者说的不是一件事。** 那条拒绝的是
/// 把码画成**字符块**——69 列 × 35 行塞不进 80×24，换 2×2 象限块又会被终端的字符宽高比
/// 压成 2:1 而读不出来。本函数交出的是一段 SVG，给的是一个**会把它画进像素**的调用方，
/// 终端的宽高比与行数在那里都不成立。
///
/// 两种形态都把 SVG 放 stdout：它就是这条命令的结果，`> code.svg` 要拿到的正是它。
/// `--json` 包成一行对象，因为那个开关声明的是「调用方是程序」，而程序要的是一个能
/// 解析的信封，不是一段裸文本。
pub fn render_qr(svg: &str, json: bool) {
    if json {
        let payload = serde_json::json!({ "svg": svg });
        super::emit_line(&payload.to_string());
        return;
    }
    println!("{svg}");
