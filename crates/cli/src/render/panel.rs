...
//!
//! ⚠️ **不要与 [`super::stream`] 混起来**：那是 `swarmdrop watch` 的事件流。
//! 两者是不同的东西——面板服务于「此刻我想看一眼」（重绘全量快照、给人看、走 stderr），
//! 事件流服务于「有变化就告诉我」（增量推送、给程序看、走 stdout）。
//! 本模块曾叫 `render::watch`，与那条命令重名，改名正是为了让这两件事在文件名上就分得开。
//!
