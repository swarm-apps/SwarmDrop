...
pub enum TransferDataFrame {
    ...
    ///
    /// 这个 tag 是 `/3` 引入的，曾经必须按协商结果决定发不发——v2 的解码器遇到未知 tag
    /// 直接判协议错误并中止会话，向 v0.12.0 及更早的对端发一帧就等于打断整次传输。
    /// `/2` 与 `/3` 的注册随 bao chunk group 变更整体摘除后只剩一个协议名，流控恒开，
    /// 那个判据（原 `WindowPacer::for_protocol`）也随之消失。
    Window {
        session_id: Uuid,
        epoch: i64,
    },
