...
impl TransferManager {
    ...
    /// 传输域事件端口。
    ///
    /// 给**需要发领域事件的编排**用：收件箱的归档与删除是编排函数
    /// （`inbox::archive_inbox_item` / `inbox::delete_inbox_item`），它们收一个事件 sink
    /// 参数，而宿主手里现成就有 manager 时经这里取最省事。
    ///
    /// ⚠️ **真正的不变量是「同一个事件总线」，不是「同一个 `Arc`」。** 这个 accessor
    /// 只在**节点已经起来**的时候才存在，而收件箱的归档与删除不依赖节点：桌面在
    /// 组装点建一次 sink 存进 Tauri state（`setup.rs`），移动端每次调用现造一个包着同一个
    /// `EventBus` 的 sink——两者都**不可能**经这里取，而两者都是对的。
    ///
    /// 所以要求写成：**这个 sink 必须转发到宿主那唯一的事件总线**。同一个 `Arc` 是达成
    /// 它的一种方式，不是它本身；照字面理解会把归档与删除的事件绑死在「节点在跑」这个
    /// 前提上，而那两件事本来不需要节点。
    pub fn events(&self) -> &Arc<dyn TransferEventSink> {
        &self.events
    }
