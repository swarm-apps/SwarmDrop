...
export const config: WebdriverIO.Config = {
    ...
    //
    // 同上：向导给的是通用浏览器 boilerplate（browserName: 'chrome'），native 模式下
    // Tauri service 只认 'tauri' / 'wry'，手动改成正确的 capability。
    capabilities,
    ...
    logLevel: process.env.SWARMDROP_DEMO_RECORDING === "1" ? 'warn' : 'info',
    //
    ...
    // 注意：官方 create-wdio 向导会在这里多生成一个 'tauri-plugin' service 条目，但
    // @wdio/tauri-plugin 只是前端集成（main.tsx 里 dev-only import + Rust 侧
    // tauri-plugin-wdio 注册），不是 WDIO service，加进 services 数组会导致
    // "Couldn't find plugin tauri-plugin service" 报错，已手动去掉。
    services: [[
        'tauri',
        {
