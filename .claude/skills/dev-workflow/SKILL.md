...
| [web-app-frontend.md](../../../dev-notes/knowledge/web-app-frontend.md) | Web 应用区**表现层**（`docs/app/app` 的 React/Next 代码）：运行时单例只能挂 layout（以及怎么验证「切页没重启节点」）、静态导出三条硬限制（没有 `redirect()`、运行时 ID 不能进路由段、`useSearchParams()` 要 Suspense）、basePath 与 `next/link`、自研 `create-store` 的 selector 约束（`check:zustand-access` 不覆盖 docs/）、Next Dev Tools 浮标挡住底部导航这类手测坑。**改 `docs/app/app` 下任何 React 代码前必读** |
