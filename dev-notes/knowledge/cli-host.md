...
## 谁起节点谁定能力上限——版本偏差是常态，不是异常（2026-08-22）

三个事实叠起来会得到一个反直觉的结论，**做任何「外部程序调 swarmdrop」的集成前必须先懂它**：

1. **数据目录按用户走，不按可执行文件走**（`ProjectDirs::from("com","yexiyue","swarmdrop-cli")`）。
   同一台机器上的两份安装共享身份、已配对设备表、库、socket 与锁文件。
2. **本地通道刻意没有版本协商**（`ipc.rs` 模块文档：「这是内部机制，不是对外 API……动词集
   就是命令面的映射，可以随时改」）。它的前提是**两端同一个 build**。
3. `single::acquire` 保证的是「一份数据目录一个节点」，**不是「一个版本」**。

⇒ **先起节点的那个进程定下能力上限，之后任何版本的调用都只是它的客户端。**
新客户端向旧常驻节点发一个它不认识的动词，serde 直接拒绝——客户端再新也没用。

**这不需要两份安装就会发生**：`swarmdrop update` 换掉可执行文件，常驻节点还跑着旧代码，
直到有人重启它。
...
| 方向 | 何处报 | 退出码 |
|---|---|---|
| 旧节点读不懂新请求 | `serve_connection` 的解析失败分支 | 2（usage） |
| 新节点的响应旧客户端读不懂 | `ipc::request` 的解析失败分支 | 3（node unavailable） |

⚠️ **两条都落在一个「已经有别的含义」的退出码上**，所以消费者按码给的提示对这两种情况
一律是错的：2 会让人去改参数（他没有参数可改），3 会让人去启动节点（节点正跑着）。
措辞是唯一的区分依据——两句里各埋了一个稳定短语（「常驻节点无法解析这条请求」/
「多半是常驻节点比这条命令新」），外部集成靠它们识别。

### `status` 带 `daemonVersion`，让偏差可被检测

0.8.0 起 `swarmdrop status` 的载荷带上应答进程自己的版本
（`ipc::with_daemon_version`）。两条判据不能破：

- **追加字段，不要包一层。** 客户端把载荷当 `Value` 读、只取认识的键，所以追加对旧客户端
  无害；换成 `{ node, version }` 会让旧客户端要的**每一个**键都落空——恰好把「新节点 ×
  旧客户端」从「大体能用」变成「status 整个坏掉」，而这个字段存在的目的正是缓解那个组合。
  护栏测试 `ipc::tests::version_stamp_leaves_every_existing_field_alone`。
- **判「旧」时「节点在跑」这个前提不能省。** 无节点时 `cmd::status` 用一份
  `NetworkStatus::default()` 顶上，它同样没有版本字段——少了这个前提，每一台没起节点的
  机器都会被告知「你的节点太旧」。护栏测试 `render::status::tests::no_node_is_not_a_version_problem`。

**不要把版本号加进 `NetworkStatus`**：那是核心的类型，桌面 / 移动 / Web 都吃它，而这件事
只对本通道有意义。

### 外部宿主解析 `swarmdrop` 时：**PATH 必须排在自带副本之前**

这条看起来是反的（自带的那份是测过的版本），但由上面的结论直接推出：

- 已经有节点在跑时，**客户端版本不决定任何事**，捆一份新的买不到能力；
- 没有节点时，捆绑副本会去**起**一个节点——而那正是把用户终端里的 `swarmdrop` 弄坏的
  那一刻（它此后连不上自己起不了的那个 daemon）。

所以顺序是 `显式覆盖 → PATH → 自带副本`。自带副本只服务「什么都没装」的人，而那种人
没有可冲突的对象。`dsh-swarmdrop` 的 `src/cli.ts` 是这条的参考实现，护栏测试
`cli.test.ts` 的 `prefers the user's own install over the bundled copy` ——**它先断言
node_modules 里确实有捆绑副本**，否则那条断言会退化成永真。

⚠️ 消费侧判「节点不报版本 ⇒ 它更旧」时**要带版本下限**：两个 0.7.1 互相通话时谁都不报
版本，读作偏差就会对着一台完全正常的机器报警。判据是「问的这一方是否已到会盖版本戳的
版本」（`dsh-swarmdrop` 的 `DAEMON_VERSION_SINCE`）。CLI 自己不需要这条——能做这个判断的
binary 必然 ≥ 0.8.0。
...
2. **没有布局与对齐**。一屏里多条进度条要成列、终端一窄就得截断，自写版本得自己算宽度。
   （⚠️ 这条**曾经写作「没有速率与剩余时间」**——那是选型时的理由，但 indicatif 估的
   速率不能用，判据见下一节。）
...
### ⚠️ 速率与剩余时间**必须由核心给**，绝不能用 indicatif 的 `{binary_bytes_per_sec}` / `{eta}`（2026-08-22）

症状：CLI 往手机发文件，手机上显示 20 MiB/s，**同一次传输**在 CLI 这边显示 200 MiB/s
——整整多一位。

**两端的字节流不可能差 10 倍**，这一点由代码保证：发送端每 `WINDOW_CHUNKS`（16 块 = 4 MiB）
停下来等一帧 `Window` 确认（`actor/sender.rs` 的 `WindowPacer` + `sync_window`），
接收端那条有界队列也正好一窗。稳态下两端最多差 4 MiB。所以**差的一定是显示，不是数据**。

真凶是 indicatif 的估算器（`Estimator`，双重指数平滑，时间常数 15 秒）配上本程序的驱动
方式——**进度条是在收到第一帧的那一刻才建的**：

- `RemoteProgress::on_frame` 里 `get_or_insert_with(Progress::new)` 之后**立刻**
  `set_position`，建条与第一次记录只差几微秒；
- `transfer watch` 的面板更糟：它每轮对一条**已经传了一半**的会话首次 `set_position`，
  第一个样本就是「几百 MB 在几微秒内完成」。

估算器于是以一个天文数字开局，再花几十秒衰减回真值。实测（首帧 512 MiB、真实 20 MiB/s）：
第 8 秒显示 201 MiB/s，第 20 秒仍有 47 MiB/s。首帧带的字节越少污染越短——普通发送 2 秒内
就收敛，所以这个 bug 只在**续传**与**面板中途接进一条会话**时显形，容易被当成偶发。

**正确做法**：两个数都取核心那一帧里的 `speed` / `eta`（`ProgressTracker` 的 3 秒滑窗，
`crates/transfer/src/progress.rs`），CLI 只排版：

- `render::send::rate_and_eta(speed, eta)` 是唯一的排版入口，结果写进 `{msg}`；
- 模板里**不许出现** indicatif 的速率 / ETA key，`render/send.rs` 与 `render/panel.rs`
  各有一条测试钉住这一点（断言模板字面量不含 `per_sec` / `{eta`）；
- 跨进程那一段要把这两个字段带过去：`send` 走 IPC 帧的 `speed` / `eta`，
  `transfer watch` 走 `ProgressCache::annotate`（**投影里没有这两列，也不该加**——
  那是存储的投影，速率是只在传输进行时存在的派生量）。

**第二条理由与准确性无关**：桌面、移动、Web 显示的都是核心那个数，而用户看的常常是同一次
传输的两端（这边发、手机上收）。CLI 自己估就必然与另一端对不上——用户报的正是这个。

两条排版规则：**停滞时给「—」不给「0 B/s」**（核心把 speed 归零表达的是「一个滑窗内没有
新字节」，接收方正在发布收齐的文件时就长这样，报 0 B/s 会让人以为传输死了）；
**ETA 必须防住 NaN / 负数 / 溢出**（`Duration::from_secs_f64` 对这三类是 panic，而值来自
另一个进程——一个进度条不该有能力打断一次正常的传输）。

**相关文件**：`crates/cli/src/render/send.rs`（`rate_and_eta`）、`crates/cli/src/render/panel.rs`、
`crates/cli/src/runtime/progress.rs`（`annotate`）、`crates/cli/src/runtime/transfer.rs`（`TransferFrame`）
...
**规则 1 下允许名词形态：`mcp`**（2026-08-21）。规则 1 写的是「平铺动词」，而 `mcp`
命名的东西没有对应的常用动词——它是**本程序的一种运行形态**，与 `start` 平级
（`start` 是「以常驻节点形态运行」，`mcp` 是「以 MCP server 形态运行」）。
**判据（防止它变成拉平一切的先例）**：这个名词命名的必须是本程序的一种运行形态，
而不是一个可列举的集合。`device` / `invite` / `inbox` / `transfer` 都是集合，
集合上的动作必须带动词，规则 2 一步不让。

`watch` 本身是个动词，落在规则 1（对象是**本程序自己的事件流**，单例），无需豁免。
...
| 入口 | 行为 | 命令 |
|---|---|---|
| `NodeAccess` | 有常驻走通道，否则起临时节点 | `send` · `invite create` · `invite use` · **`mcp`（节点长驻，见下）** |
...
| 自成一档 | **不起节点，但要推送而非查询** | **`watch`（见下）** |
| （都不用） | 只碰本地文件系统 | `inbox export` 的文件复制部分 |
| 自成一路 | 节点生命周期本身 | `start` / `stop` / `status` |

**`mcp` 是 `NodeAccess`，但节点持有到 server 退出**（2026-08-21）。其余 `NodeAccess`
命令是「起节点 → 干活 → 关节点」，它不是：每次工具调用现起现关的话，**每一次发送都要
重连引导节点、重做 NAT 探测**——秒级调用被拖成数秒，而 agent 的一轮对话里可能连着调好几次。
`runtime/` 本就不得假设调用方是一次性命令（`standalone-cli-host/design.md` 的 D11），
这是那条约定的第一个长驻消费者。

⚠️ **自持节点时它必须摆成「常驻形态」，不能只是「起了个节点」。** 这条第一版漏了，
三种失败一起来且没有一种会报错：① 它握着单实例锁数小时而不建通道，于是同机每一条命令
都一路重试到「另一个 swarmdrop 进程正在启动，等待超时」；② `swarmdrop watch` 的判活
永远为假，订阅停在直读记录的基线上；③ 那个节点在线、可达、已配对，却**一个文件都收不下**
——而「接收不是一条命令」正是本产品的基本承诺。

装配因此收在 **`runtime/daemon.rs`**（绑本地通道 + 起被动接收 + 开配对确认台 + 服务循环），
`start` 与 `mcp` 共用。此前它整段写在 `cmd/start.rs` 的 `run()` 里，也就**隐含了「只有
`start` 会长驻」这个假设**——那正是 `runtime/` 不得做的那种假设，第二个长驻消费者出现时
当场显形。护栏是 `tests/mcp_host.rs`：本仓唯一一条真的会启动 P2P 节点的测试，判据取
「通道文件出现了没有」而不是计时。

**被动接收与配对是两扇不同的门**：`mcp` 关的是配对窗口（邀请是一次性凭证，被抢走就消耗
掉了，而 server 后面没有人在看），接收照收——把后者也关掉不会让任何东西更安全，只会让
这个节点变成一台收不到东西的设备。

**`watch` 不启动节点，但它要的是推送不是查询**——所以它既不是 `RecordAccess`（那一档的
形态是「问一次，答一次」）也不是 `DaemonAccess`（那一档没有节点就报错退出）。它的形态是
**「有常驻节点就经通道订阅，没有就等它出现」**：先给一条直读本机记录拼出来的基线，
然后等；节点关停后推一条 `nodeUnavailable` 继续等，不退出。

这一条不是便利而是契约：它让调用方**不必关心自己与节点的启动顺序**。少了它，宿主
（agent harness 的插件）就得自己写一套重试与竞态处理，而那正是这条命令该替它做的事。
...
⚠️ **`watch` 是唯一不能加进那张表的**：表里的 `run()` 用 `.output()` 等进程结束，
而它永远不结束——加进去会让整个测试永久挂住（CI 上只看得到一次超时）。它有一条独立用例
`watch_without_a_node_emits_a_baseline_and_starts_nothing`：spawn + 读第一帧 + kill，
判据仍是身份文件在不在。`mcp` 同理（它归 `NodeAccess`，本来就不在那张表里）。
...
4. **速率与剩余时间同样要经这条缓存过来**（2026-08-22 补）。它们在投影里没有字段，
   由 `ProgressCache::annotate` 在 JSON 化之后标上去。第一版没做这件事，面板只好让
   indicatif 自己估——而这个面板恰好是那个估算器最坏的用法（每轮对一条已经传了一半的
   会话首次 `set_position`），显示值高出一个数量级。判据见「速率与剩余时间必须由核心给」。
...
### 内核有的能力，命令面不一定有——`inbox search` 就漏了一年（2026-08-22）

`runtime::inbox::search` 与 `Request::InboxSearch` 早就存在，daemon 也一直在处理它——
**MCP 的 `search_inbox` 用的就是它**。缺的只是 `InboxAction` 上的一个动词，于是命令行
用户与外部宿主（`dsh-swarmdrop`）都只能拉全量列表自己过滤，而那样**搜不到正文与文件名**
——恰恰是「用关键词找一条收到的东西」时唯一记得住的部分。

判据留给下一个人：**加了 `runtime::*` 的取数函数与 `Request::*` 的变体之后，问一句
「命令面有没有对应的动词」**。MCP 与命令面是两个消费者，只接一个不会有任何东西报错。

⚠️ 加动词时注意 `every_optional_target_makes_the_command_interactive` 那条护栏：它按
「只给命令名」构造最小调用，而 `inbox search <QUERY>` 的 QUERY 是必填——那个构造对它
不合法，判据里因此显式跳过带必填参数的命令。**这不是给它开后门**：`--limit` 缺席时也
不问人（默认归内核），它本来就不该算交互命令。
...
## 事件订阅面 `swarmdrop watch`（2026-08-21）

给宿主程序（agent harness 的插件、脚本、服务单元）的推送式订阅：收件箱 / 传输 / 设备
三类变化，NDJSON。行为契约在 `openspec/specs/cli-event-stream`，实现分工写在
`crates/cli/src/runtime/watch/mod.rs`。

### 它与 `transfer watch` 是两条**语义相反**的流，别互相搬写法

| | `transfer watch` | `watch` |
|---|---|---|
| 受众 | 人 | 程序 |
| 覆盖 | 只有传输 | 收件箱 / 传输 / 设备 |
| 形态 | 固定间隔重绘**全量快照**，带热键 | **增量推送**，一次基线之后只发变化 |
| 去向 | **stderr**（它是过程信息） | **stdout**（它就是命令结果） |
| 节点起落 | 每次刷新各自判断 | 跨起落存活，节点回来自动接上 |

判据一句话：**面板服务于「此刻我想看一眼」，订阅服务于「有变化就告诉我」**。

⚠️ 面板**至今仍是轮询**，尽管通道上如今确实有推送能力（`Request::Subscribe`）。
差别不是能力而是形态：面板每帧要的是全量快照（一屏进度条要对齐、按阶段分组），
用订阅就得在那边再维护一份由事件折叠出来的镜像——那份镜像与真相分叉时，屏幕上会显示
一条其实已经结束的传输，而没有任何东西会纠正它。

渲染模块因此也改了名：`render::watch` → **`render::panel`**（面板），新的
**`render::stream`** 才是这条订阅。旧名与命令名重叠，是这次改名的全部理由。

### 进度样本带速率与 ETA，**这两个字段不是可有可无的**（2026-08-22）

`ProgressSample` 除聚合字节数外还带 `speed`（B/s，领域的 3 秒滑窗）与 `eta`（秒，可为
`null`）。加它们的理由不是「顺手」：消费方手里只有一串按秒降频的快照，照着它反推速率就要
各自维护一个时间窗口——**本仓的两个渲染面都在这道题上栽过**（判据见「速率与剩余时间必须
由核心给」）。把这道题留给每个消费方，等于把同一个坑复制到仓库外面，而这条流的记录会被
跨月留存、事后回放。

三条语义要一起转述给消费方（都写在字段的文档注释上）：`speed == 0` 是「一个滑窗内没有新
字节」（停滞）而不是「速率恰好为零」；`eta` 算不出来是 `null`，**不折成 0**（0 秒是「马上
就好」）；两者**同源同帧**，不会出现「12 MiB/s · 剩余未知」这种自相矛盾的行。

降频折叠取的是**最新一帧**（`fold::Coalescer` 用 `insert` 覆盖），这对速率恰好是对的——
领域那个数已经是滑窗均值，在这一层再平均一次就是双重平滑。

⚠️ 新增字段**不递增 `SCHEMA_VERSION`**（判据写在那个常量上：消费方被要求忽略不认识的
字段）。同一条事件的人读形态（`render/stream.rs`）也要一起改，否则 `--json` 那行与人看的
那行说的不是同一句话。

### wire 必须是订阅面自己定义的窄结构，**绝不转发 `CoreEvent`**

三条独立理由，任一条都足够：

- **会泄露配对凭证**：`PairingRequestReceived` 把 `PairingRequest` flatten 进事件，
  而 `PairingMethod::Invite` 携带 128bit bearer capability **明文**——那个类型的注释
  自己写着「明文不落盘」，而这条流的终点正是消费方跨月留存的日志。
- **会泄露正文**：文本条目的 `title` 就是正文的前 160 字节，所以也不能直接复用
  `InboxItemSummary`（订阅面的 `InboxEntry` 就是为此不带 `title`）。
- **`CoreEvent` 的 serde 实现在生产里从未被执行过**——四个宿主全是 match 重映射。
  直接透传等于让一个未经检验的 `tag` + `flatten` 组合当场变成对外契约。

### 序号由**客户端**发，版本挂在每一条上

序号的作用域是「这条订阅」，而订阅跨节点起落存活——服务端随节点起落换人，发不出连续的号。
版本挂在每条而不是靠握手协商，因为消费方会**持久化**这些事件：握手覆盖不了「三个月前
写下的那一行」。

⚠️ 客户端在这条流上是**转发者不是解释者**：它把服务端给的 JSON 对象直接盖上 `v` / `seq`
就转发，**不反序列化成 typed 值再序列化**。常驻节点完全可能比客户端新（升级 CLI 不会重启
常驻节点，`swarmdrop update` 之后尤其如此），反序列化会在那一刻失败，把一条本可正常流过
的记录变成一次订阅中断。

### 背压：入队非阻塞 + 出队可阻塞 + 丢弃如实上报

```mermaid
flowchart LR
  P["领域 publish"] -->|try_send，满了就丢| Q["有界队列"]
  Q --> L["serve 循环"]
  L -->|阻塞写，慢就慢| S["本地套接字"]
  S --> C["客户端 stdout"]
...
三段缺一段就把代价转嫁到不该承担它的地方：

- **入队非阻塞**（`CliEventBus::subscribe_lossy`）：`publish` 被 await 在传输的收发块
  簿记里，阻塞它等于让一个**旁观者**拖慢正在进行的传输——那条回路的终点不在本程序手里。
- **出队可阻塞**（`FrameSink::send`）：serve 循环是这条订阅专属的任务，挂住不影响任何
  别的东西；阻塞正是把压力顶回上一段的方式。
- **丢弃如实上报**：变成一条 `truncated`，绝不静默。消费方分辨不了「流结束了」与
  「我漏了一段」，而它会把这段记录长期留存。

⚠️ **只有进度算「采样」，丢了不上报**；其余全是边沿类。判据写成「只有进度可丢」而不是
逐条列举边沿，是为了让**下一个人新增的事件默认落进边沿一侧**——反过来写会让它静默消失。
判据的归属地是 `watch::event::is_edge`，`subscribe_lossy` 收它当参数：哪些算采样是订阅面
的语义，不是事件总线的。

### `FrameSink` 的两个方法是两种投递策略，判据是「调用方是谁」

| 方法 | 阻塞调用方？ | 丢帧？ | 调用方 |
|---|---|---|---|
| `try_send` | 绝不 | 会，静默 | `select!` 的**分支体**（传输准备的进度） |
| `send` | 会 | 不会 | 一条订阅**专属**的任务 |

不是「一个安全一个不安全」：分支体挂住会让同一个 `select!` 里真正干活的 future 得不到
轮询，**常驻节点上别人的传输就停在那儿**；而订阅任务挂住什么都不影响，阻塞正是背压。
在订阅上用 `try_send` 会让边沿事件在那里无声消失。

### 重连必须退避，哪怕「看起来连得上」

`follow` 一轮结束就回去重连，而 `is_alive` 在两种情形下仍为真：**新客户端撞上没重启的旧
常驻节点**（它不认识 `subscribe` 这个动词，直接回一个用法错误），以及服务端拼基线持续失败。
不退避就是一个满速死循环——烧满一个核、往宿主 stdout 灌 `nodeUnavailable`、并且每转一圈
都往常驻节点的订阅者表里塞一个新 `Watcher`（那张表只在下一次广播时才清理）。

重连从来不急：节点关停之后总要过一会儿才回来。

⚠️ 还有一条相邻的竞态：`is_alive` 说在、`connect` 说不在（节点正好在这两句之间关停）。
那时基线已经「让给服务端」了，若直接 `return`，消费方**一条事件都收不到**——而 spec 要求
「订阅建立时必须先有一条基线」。判据用 `seq == 0`（还什么都没发过），补一条本地基线。

### 丢弃计数只能算「真会出现在这条流上」的事件

领域事件里有一大半与订阅面无关（`PrepareProgress`、`NetworkStatusChanged`、配对请求……），
它们进队列只是路过。把它们的丢弃也算进 `truncated`，是在告诉消费方「你的记录有个洞」——
而那个洞从来不存在。**发一个大目录时 `PrepareProgress` 就能刷满队列**，这不是罕见场景。

判据直接问 `translate`（返回 `None` 就是与本面无关），**不另列一张变体表**：两张表迟早分叉，
而分叉的表现正是那条凭空的截断。

### 长驻的流式命令不能用 `println!`

Rust 启动时把 `SIGPIPE` 设成忽略，于是往一条已关闭的管道写会让 `write` 返回 `EPIPE`，
而 `println!` 对此的反应是 **panic**（退出码 101）。`swarmdrop watch` 唯一的产出就是往
stdout 写，而宿主结束订阅时先关读端是它的常规动作——101 会被读成「它崩了」并触发自动重启。

改用 `writeln!` 并把 `BrokenPipe` 当作**正常终点**（返回值一路传回订阅循环，收摊退 0）。

⚠️ **不要用「把 `SIGPIPE` 恢复成默认处置」一劳永逸**——那正是 Rust 忽略它的原因：
本进程跑着 P2P 栈，Linux 上对一条已关闭的 TCP 连接 `write` 同样会抬 `SIGPIPE`
（Rust std 只在 Apple 平台给 socket 设 `SO_NOSIGPIPE`），恢复默认等于让任何一次对端断连
都可能直接杀掉节点。这条弯路走过一次。

### 三条容易写错的收敛规则

1. **基线的传输部分必须经 `TransferUnfinished` + `ProgressCache::overlay`，不直连库。**
   发送方向的进度整条传输期间都不落库，直连的症状是「进度一路停在 0%，暂停的瞬间跳到
   43%」——`transfer watch` 第一版栽过。
2. **设备去抖靠差分而不是时间窗。** `DevicesChanged` 的高频来源是**内容相同的重复**
   （每次 ping 成功都推一遍全量，且用的是 `DeviceFilter::All`——路过的陌生设备也在里面）。
   差分把它们全部吸收，同时一次真正的上下线一帧不差地立刻发出。时间窗会**延迟**一条
   边沿事件，那是反的。
   ⚠️ 差分前要**按标识排序**：两条来源的次序都没有契约，次序一抖就等于每次 ping 都推
   一条内容完全一样的变化事件。
   ⚠️ 收敛器要用**基线里那张表**开局（`Coalescer::seeded`），否则节点起来后消费方会在
   基线之后立刻收到一条什么都没变的「变化事件」。
3. **会话进终态时要丢掉攒着的那帧进度。** 不丢的话它会在终态之后再冒出来，消费方按事件
   顺序重建时，一条已经结束的传输会退回「传输中」。

### 「配对成功 / 解除配对」要向节点现取设备表

`PairedDeviceAdded` / `PairedDeviceRemoved` / `DeviceRenamed` 只带一个 `peer_id`，
全量表得现查（`watch::event::invalidates_devices` 标出这三条）。
**不能指望下一条 `DevicesChanged` 兜住**：那条由网络事件驱动，而解除一台**离线**设备的
配对不产生任何网络事件——不补这一下，那次解除会在订阅上无限期不可见。

### 长驻命令的退出语义：`SIGTERM` 必须等同 `SIGINT`，且退 0

`tokio::signal::ctrl_c()` 在 Unix 上**只接 `SIGINT`**，而这几条命令的主要调用方发的都是
`SIGTERM`（agent harness 的 `terminate()`、`systemctl stop`、Node 的 `child.kill()`）。
只接 `SIGINT` 会让**最常见的正常收摊路径**走不到清理、退出码非零——而消费方把非零读作
失败并触发重启或告警。实现在 `runtime::signal::shutdown`，覆盖 `watch` / `mcp` /
前台 `start`。

⚠️ `crates/bootstrap/src/util/signal.rs` 有一份**逐字同构**的实现，两处互相指着对方。
没合并是因为没有合适的共享落点（`crates/host` 要过 wasm 门禁，`tokio::signal` 进不去）。

⚠️ **信号监听器必须建一次、反复 `recv`，绝不能写在 `select!` 的一支里。**
tokio 的信号接收端是个 `watch` 订阅，新订阅从**当前版本**起算，看不到自己出生前的那次通知；
而 OS 层的处理器一经安装就不再卸载，默认处置（终止进程）也不会兜底。**两头都不接 = 信号
消失**，表现是「`systemctl stop` 没反应，等满超时才被 SIGKILL」，日志里什么都没有。

这条现在由 `runtime::signal::Shutdown` 承载（要先 `listen()` 才能 `recv()`，建在循环外是
它唯一自然的用法）。此前三个 `select!` 循环各自在分支里现建现丢；`cmd/invite.rs` 是唯一
写对了的地方，它的注释也早就写着这件事——只是没有被提炼成类型。

⚠️ 另一半是**注册时机**：注册发生在那个 future 第一次被轮询时。放到「拼基线」之后的话，
那段读库期间收到的 `SIGTERM` 会按默认处置直接杀掉进程。

### ⚠️ `swarmdrop mcp` 收到信号时**不能靠 `return` 退出**

装上信号处理之后它反而挂死了——比之前退非零更糟。根因与信号无关：**MCP 的 stdio 传输把
stdin 交给了 tokio 的阻塞读任务**，而被信号叫停时宿主仍握着 stdin 不放（它才是那条流的
主人）。`main` 返回时运行时析构会等所有阻塞任务收尾，那次读永远不返回。

所以那条路径上清理做完之后直接 `std::process::exit`（`cmd/mcp.rs` 的 `exit_now`）。
两件事跟着来：**退出码要自己算**（与 `dispatch` 共用 `cmd::finish`，否则同一类失败会长出
第二种措辞），**stdout / stderr 要显式刷一次**（`process::exit` 不跑 Rust 的刷新）。

常规路径（宿主关掉 stdin）不走这条：那时阻塞读已经返回，运行时析构得掉。

护栏 `tests/mcp_host.rs::sigterm_ends_the_process_even_while_the_host_holds_stdin`——
它必须**一直不 drop 那个 stdin 句柄**，撒手就走到正常路径，什么都测不到。

## dsh（DeepSeek Harness）的三条硬约束（2026-08-20 一手核实，`0.1.0-rc.8`）

记在这里是因为它们决定了本仓这条订阅面**为什么长这样**，而不是长成一条「浏览器直接问
CLI」的捷径。

1. **Typert Remote 对第三方不可用。** 生成流水线跑在 dsh **根构建**里（typert generator
   以 Host aggregate 为唯一 `ts.Program` 种子），仓外的包生成不出 `typert.remote-client.*`；
   而「Client Remote 拒绝挂载缺少严格 codec 的 SRC 描述符」，开发回退也堵死。
2. **API Proxy 的方法集是封闭的。** `RpcMethodMap` 只注册 `SessionsApi` / `HostApi` /
   `EventsApi` 三个领域接口，第三方加不进去。
3. **共同根因是事件溯源。** dsh 的 Session 事件流按 `seq` 升序回放必须能确定性重建 UI，
   Conversation Node 的 `start`/`update` 不得依赖「只存在于实时内存中的状态」。
   一条旁路 RPC 会直接破坏它：刷新页面、翻历史分页、多客户端同看一个会话时，那部分 UI
   就重建不出来。**这不是第三方待遇差，是架构不变量**——dsh 自己的包也走同一条路。

推论：插件 Client 半边的数据只能从 Session 事件流 fold。代价是做不到「点开选择器的瞬间
现查一次」，收益是**carrier 无关**——browser / in-process / Electron 三种 carrier 下都成立，
人在外面用手机连回家里的 dsh 也照样能用。而 loopback 直连（浏览器 `fetch
http://127.0.0.1:<port>`）假设了「browser carrier + Host 在本机」，后两种 carrier 下当场不成立。

⚠️ 上游明确声明会有破坏性变更（developer preview）。**易碎的那一半全在仓外**
（独立仓 `dsh-swarmdrop`）：dsh 改了 seam 只改那个仓，本仓的配对 / 传输 / 收件箱一行不动。

## 配置面（2026-08-21）：`config` 与 `bootstrap` 两个名词

第四个宿主此前一个配置动词都没有：设备名已经持久化了却没有命令改它，接收落点只认环境
变量、不持久化，引导节点清单是编译期常量——桌面 / 移动 / Web 能改的东西它一样都改不了。
现在两个名词补上这条：`swarmdrop config`（标量）与 `swarmdrop bootstrap`（集合）。
...
| | 用户的意图 | 动词 |
|---|---|---|
| `config` | 「把这个值换成那个」 | `list` / `get` / `set` / `unset` |
| `bootstrap` | 「加一条」「撤一条」 | `list` / `add` / `remove` |

把引导清单塞进 `config set` 会迫使调用方**读出整份清单 → 编辑 → 整份写回**，而那正是
`bootstrap-node-settings` 明令禁止的「持久化合并后的最终清单」（见下一节）。形状不同的
东西共用一个动词，代价是把一个已知会咬人的模型请回来。
...
环境变量保持最高优先级是刻意的：命令行宿主常跑在脚本与服务单元里，那些地方设一个环境
变量比维护配置文件自然得多；新增持久化配置不等于把它降级。当前只有接收落点有环境变量
（`SWARMDROP_RECEIVE_DIR`）。

⚠️ **难点不在优先级而在告知。** 一个只显示「当前值」的设置界面，无法解释为什么用户刚改
的值没生效，用户会反复改那个不生效的输入框。所以 `--json` 读面对每一项给四个字段：

| 字段 | 含义 |
|---|---|
| `value` | 当前**生效**的值 |
| `source` | `env` / `config` / `default` |
| `configured` | 持久化配置里的值——`source == "env"` 时它就是**那个被压住的值** |
| `overriddenBy` | 压住它的环境变量名 |

写入结果同理是三态而不是一个成功位：`applied` / `pendingStart` / `overridden`。
环境变量存在时写入**仍然成功**（它是对未来的声明——用户很可能正在为「以后不带那个变量
跑」做准备），但必须当场说清此刻不生效及原因。

### 有节点时**必须走 IPC，不能直写**

这是本块最容易写错的一处。常驻节点内存里持有设备名（identify 的 `agent_version`）与接收
落点。绕过它直写文件，磁盘与内存会分叉——**用户改完名字，对端看到的还是旧的**，而文件里
已经是新的，重启之后才「莫名其妙地」生效。

分叉点只有一个，就是既有的 `RecordAccess::query`：连得上常驻节点就交给它（它顺带换掉内存
里那份），连不上就本进程直写。**无节点时绝不为写入拉起一个**——`cli-command-surface` 的
「只有需要联网的命令才启动节点」直接管到这里，护栏是 `tests/without_a_node.rs` 的
`writing_settings_never_starts_a_node`（从进程外判断 `identity.json` 在不在）。

改名走 core 的 `device_name::rename_device`，**不自己拼 `agent_version`**：那条编排把
「落盘 → 本机 `OsInfo` → identify → 事件」四步的顺序与失败语义定死了（持久化在最前，
因为「名字自己回滚」是最难向用户解释的状态），宿主自行拼装等于把那份顺序再猜一遍。
接收落点则是把 `runtime::receive::ReceiveDir`（一个 `RwLock<PathBuf>` 句柄）换掉——
**它必须是句柄不是值**，否则改落点只能靠重启节点，而重启节点会断掉正在进行的传输。
...
- `add`：地址在 `removed` 里 ⇒ 从 `removed` 拿走；否则进 `custom`。
- `remove`：地址在 `custom` 里 ⇒ 从 `custom` 拿走；否则（它是内置的）进 `removed`。

⚠️ **不对称是必须的。** 把「撤销一条内置项」实现成「把内置清单复制进 custom 再删一条」会
复现一个已知故障：版本更新换掉内置地址时，老用户的 custom 里躺着一份旧地址快照，新地址
永远到不了他手上，故障形态是「升级后突然连不上」且用户无法自查。

**允许清空到零条**，不设下限：只在局域网内用 SwarmDrop 是合理场景，而「至少留一条」会在
用户想整体换掉内置节点时挡住他（他得先加后删，顺序错了就被拒）。代价由 `bootstrap list`
与 `status` 如实显示承担——两处都会在清单为空时把后果说出来（「只能在局域网内发现设备」）。

**运行时的落地是「先撤再登记」而不是只加或只减**：候选表的 upsert 对地址是**累加**，没有
「去掉一条地址」的原语，而同一个 peer 常有多条地址（内置的 TCP + QUIC 就是同一台）。

### 没有节点时怎么校验地址

`bootstrap add` 的校验必须零网络往返，而它的第三条规则是「本端点装配了这个传输吗」——那
是内核事实。有节点时问 `Endpoint::supported_transports()`；**没有节点时问
`swarmdrop_core::runtime::planned_transports(Native)`**，它走的是与真 bind 同一个装配函数
（`with_transports`），由 core 的 `planned_transports_match_a_real_endpoint` 钉住两者相同。

⚠️ **本机身份读不出来时跳过「指向本机」那条规则，绝不现场创建一个。** 用
`load_identity()` 而不是 `load_or_create_identity()`：后者会在全新机器上写出
`identity.json`，而那正是 `tests/without_a_node.rs` 用来判断「这条命令起没起过节点」的
标志——一条只读本机记录的命令产出它，那条护栏就永远绿了。（跳过是正确的而不是将就：
全新机器还没有身份，节点下次启动才随机生成一个，它不可能与用户此刻粘的地址相同。）

### 配置文件：`settings.json`，读坏了**不降级**

数据目录下一个 JSON 文件，装接收落点与引导节点的两个集合。**设备名不在里面**——它是
`DeviceConfig` 端口的磁盘格式（`device_config.json`），四端共用一个实现；把这两项加进那个
端口，移动端与桌面端就要实现两个它们用不上的方法。

两条规则与 `JsonFileIdentityStore` 同体例：**原子写**（同目录临时文件 + rename），
**解析失败返回 `Err` 而不回落默认**。静默回落意味着用户自己加的那条中继在一次坏块之后
无声消失，而他只会发现「跨网突然连不上」，没有任何线索指向这个文件。
「文件不存在」不算解析失败——那是全新机器的正常形态。

### 新动词与旧常驻节点

升级 CLI 不会重启常驻节点，于是「新客户端 × 旧节点」是个真实窗口：旧节点认不得
`config_set` 这类新变体，`serde` 解析失败。服务端那条兜底措辞因此指向最可能的成因
（「多半是它比这条命令旧——先 stop 再 start」），而不是一句指向用户输入的「无法解析请求」
——用户的输入完全正确。
