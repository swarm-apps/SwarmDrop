...
#### 体积上限 10KB gzip，且**注释也算在内**

这里曾经写着「实测 3.2KB gzip」，是个**会漂的死数字**：页面后来长出深链出口与复制说明，
...
- **`pnpm check:landing`**（`scripts/check-landing.mjs`，**在仓库根跑**，与另外三条 `check:*`
  ...
  `public/`（`docs/lingui.config.ts` 的 include 只有 `app/app`）。

⚠️ **`public/` 下的文件原样送给用户，所以那个文件里的注释也在预算里。** 本仓注释密度高，
在别处没有成本，在这里有——详细论证要放知识库（也就是这里），页面里只留结论与指针。

#### 多语言：HTML 内置源 locale + JS 字典只带另外两种

同一条「不进 Next」的约束让应用区那套 Lingui 用不了（要构建管线）。做法是把源 locale（zh）
的原文**写在 HTML 里**，`data-i18n` 标出可替换的节点，脚本只携带 en / zh-TW 两份字典。

**它的理由是「无 JS 也要可读」，不是省字节**——这条要写清楚，否则下一个人会按错误的成本
模型做决策：

- **真理由**：这页的渐进增强是**反过来**的，`#bad`（链接不完整）默认可见、脚本命中 payload
  后才藏起来。源 locale 写在 HTML 里，JS 被禁时页面仍是一屏完整可读的中文，而不是空壳。
- **省下的字节几乎为零**：实测给 `STRINGS` 补一份完整 zh 字典**只多 99B gzip**——deflate
  会把它匹配到正文里同样的中文串上。字典的真实成本是 en 555B + zh-TW 420B，**每个用户
  （含中文用户）都要下载**。

顺带一个反直觉的实测：多语言那一版改动的 +2.7KB gzip 里，**注释占 43%（约 1.16KB）、
比两份字典（约 1.0KB；en ~545B + zh-TW ~400B）还贵**。字典形态本身已经是最优解——并排
`key: [en, zhTW]`、数字索引、单字母 key 三种变体实测只差 58–121B gzip（gzip 早把重复的
key 匹配掉了），不值得拿可读性换。**要省就省注释**，而且省的是「知识库里已经写过一遍」
的那部分。

两条实现约束：

- **语言判定必须在 payload 校验之前**。脚本在 payload 非法时 `return`，而 `#bad` 那一屏
  同样要翻译。
- **`t()` 与定时器变量不要都叫 `t`**。函数声明会 hoist 到 IIFE 顶部，内层的
  `var t = setTimeout(...)` 会 shadow 掉它，`t("no-response", …)` 当场 TypeError。

语言判定与应用区 `_lib/i18n.ts` 的 `resolveLocale` 同一套规则（显式选择 > 浏览器偏好 >
源 locale），显式选择读的就是应用区写的 `swarmdrop:locale`——同域，localStorage 互通，
回访用户在应用区选过英文，这一屏就不该再用中文迎接他。

**规则只有一处刻意分歧**：三种语言都不沾时落地页给 `en`、应用区回落 `zh`。落地页的访客是
被陌生链接带来的，来源语言完全不可控。其余必须一致，两边都写着对方的存在：

- `preferredLocale` 有测试钉着（`_lib/i18n.test.ts`），落地页那份手写副本改了要同步；
- `swarmdrop:locale` 这把 key 的**所有方**（`i18n.ts` 的 `STORAGE_KEY`）注释里写明了
  「第二个读者 import 不到本模块」，`pnpm check:landing` 核对两边字面量是否还对得上。
  改名不会有任何编译错误，只会让落地页从此静默忽略用户选过的语言。

**给落地页写这份副本时，对拍出了应用区两个存量 bug**（2026-08-06 一并修掉，测试见
`_lib/i18n.test.ts`）。两个都属于「匹配成功了，只是给错了」这一类——不报错、不回落，
比「没匹配上」难发现得多：

1. **偏好顺序被匹配精度压过。** 原实现先跨 tag 把整个列表精确扫一遍、再扫主语言，于是
   靠后但精确的 tag 赢过靠前但不精确的。`["zh-CN","en-US","en"]`（Chrome 上中文用户最
   常见的列表之一）里 `en` 精确命中，**首选简体中文的用户拿到英文界面**。正确形状是
   外层按用户偏好顺序、内层才定精度。
2. **繁体变体被按主语言归进简体。** `zh-HK` / `zh-MO` / `zh-Hant-*` 全落到 `zh`，
   香港用户拿到一屏简体字。⚠️ 单元素用例测不出这条——**Chrome 会把基语言补进列表**，
   港澳用户实际发的是 `["zh-HK","zh"]`，末尾那个裸 `zh` 会在第一轮抢先命中。

配套还有两条：`zh-Hans-HK` 里 `hans` 要先于地区码判定（`-hk` 是地区不是字形），
`yue`（粤语）按繁体处理。

**改任一侧都要重新对拍**——两份实现只在单标签输入上重合，而真实浏览器从不只发一个标签。
对拍方法是把 HTML 里那段 `var lang = (function(){…})()` 原样 `new Function` 求值，
喂同一组 `navigator.languages` 与 `preferredLocale` 比对。

字典漏一条不会报错，只会让那一行**回落成中文**混在英文界面里（正是 Lingui 的 `Missing`
列在应用区拦住的那类问题），所以 `check-landing.mjs` 一并核对 `data-i18n` / `t("…")`
与两份字典的 key 是否对齐。
