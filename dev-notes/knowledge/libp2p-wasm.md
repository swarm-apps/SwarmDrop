...
| `/webrtc-direct` | `libp2p-webrtc 0.10.0-alpha` ⚠️ | 免域名免证书，但 alpha，且**不在 libp2p facade 里**（`libp2p/Cargo.toml` 只有 `webrtc-websys`，原生 webrtc 要直接依赖） |
...
|---|---|---|---|
| **私有 IP**（LAN helper）| ✅ 私有 IP 字面量豁免 mixed content | ❌ CA 不给私有 IP 签证书，物理上不可能 | ✅ certhash |
| **公网裸 IP**（用户自建）| ❌ **mixed content 拦死**（豁免只给私有 IP / `.local`）| ❌ **必须域名 + CA 证书** | ✅ **certhash，裸 IP 就行** |
...
代价只有一个：`libp2p-webrtc 0.10.0-alpha`。**这是要吃的风险，不是要绕的问题。**

> ⚠️ **这张表的地基是「WebRTC 不受 mixed content 约束」，而这条我只有间接证据**
> （它不走浏览器的 HTTP 栈；Chrome 的 LNA 文档把 WebRTC 与 fetch 分列门控）——
> **没找到明确的规范原文。必须实测。** 若此条不成立，整张表右列作废。
...
| 需要域名 + CA 证书 | **绕开** —— webrtc-direct 用 certhash |
...
⇒ 局域网场景下 `libp2p-websocket 0.46.0`（稳定）**可能就够用，不必吃 webrtc-direct 的 alpha**。

> ⚠️ **必须实测确认**。这与长期以来的 mixed content 行为相反，是随 LNA 一起改的
> （用「权限提示」换掉了「一刀切阻断」）。**文档说的和浏览器实际做的不总是一回事。**
...
1. `ws://` 私有 IP 从 HTTPS 页面到底通不通（文档说通，**以实测为准**）
2. LNA 权限提示今天会不会弹、弹的话文案与时机
3. 浏览器经 LAN helper 的 relay 能否连到第三台设备
4. 证书持久化：`libp2p-webrtc` 的 `Certificate` 有 `from_pem`/`serialize_pem`
   （`transports/webrtc/src/tokio/certificate.rs:66,76`，`pem` feature），libp2p 侧无强制轮换
   ⇒ certhash 应能跨重启稳定，但需验证 `RTCCertificate::from_key_pair` 的默认有效期
...
### libp2p 有一等公民的 `wasm-bindgen` feature
...
- **【最优先】WebRTC 是否真的不受 mixed content 约束** —— 「webrtc-direct 用裸公网 IP 服务浏览器」
  这整条路的地基。只有间接证据，**没找到规范原文**。半天可实测。
