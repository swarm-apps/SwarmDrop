...
>
> **spike 归档（2026-07-20）**：三个验证探针 `spike/iroh-web`、`spike/webrtc-direct-https`、
> `spike/net-web-smoke` 已随目录清理删除，结论均已并入本文；删除前的完整快照留存于 `archive/spike`
> 分支，需要继续 spike 时从它 checkout。下文对 `spike/*` 的引用为历史来源标注（链接已失效属正常）。
...
**已由 `spike/webrtc-direct-https` 实测坐实**（2026-07-17，Chrome）：
...
`spike/webrtc-direct-https` 逐变量隔离后锁定是版本
...
> ✅ **已实测确认**（`spike/webrtc-direct-https`，2026-07-17，Chrome）：
...
| # | 项 | 状态 |
|---|---|---|
| 1 | `ws://` 私有 IP 从 HTTPS 页面通不通 | ✅ **通**（spike，RTT 200µs；含 fetch 对照证明豁免专属私有 IP）|
...
### wasm-pack 打包与产物（spike 归档留存）

> 以下来自已删除的三个探针目录（`spike/iroh-web` / `webrtc-direct-https` / `net-web-smoke`，
> 快照见 `archive/spike` 分支）。结论对 libp2p-wasm 生产路线同样成立，摘此留存。

- **`wasm-pack build` 必须带 `--weak-refs`**：靠 JS `FinalizationRegistry` 自动回收 Rust 对象。
  不开就得对每个长生命周期对象手动 `.free()`；libp2p 的 `Swarm` / `Connection`（以及 iroh 的
  `Endpoint`）都是持有资源的长生命周期句柄，漏一个就泄漏。
- **wasm-pack 从 `Cargo.lock` 自动装匹配的 `wasm-bindgen-cli`**（实测 v0.2.126），无需硬 pin
  `wasm-bindgen="=0.2.x"`。那种 pin 是**手工串 CLI**（`cargo build` 后自己调 `wasm-bindgen`，
  schema version 对不上直接报错）链路的产物，走 wasm-pack 就不吃。
- **wasm-pack 会往 `--out-dir` 塞一个内容为 `*` 的 `.gitignore`** —— 直接吐进 `static/` 会把
  `index.html` 一起吞掉（`git check-ignore` 实测）。产物要单独放 `static/pkg/` 之类子目录。
- **`Multiaddr` 的 `Debug` 不带引号** —— 拿 `{:?}` 生成 js（如 `export const A = {:?};`）会得到
  `= /ip4/...;` 的无效语法，页面 import 直接炸。codegen 里要用 `"{}"`。

**产物体积对照**（`net-web-smoke` 冒烟壳实测，2026-07-18）：swarmdrop-net 新内核 wasm
**1836 KB 裸 / 598 KB gzip**，比 iroh spike 的 849 KB gzip **小约 30%** —— 同一浏览器落地目标下，
libp2p 路线的体积不劣于 iroh。
...
- ~~**WebRTC 是否真的不受 mixed content 约束**~~ —— ✅ **已由 `spike/webrtc-direct-https` 实证**（2026-07-17，Chrome）。
