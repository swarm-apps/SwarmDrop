...
> **当前状态：已决策暂不迁移。** 本文件保留为 Iroh 生态与迁移成本的调研记录，不再作为实施路线。
> 当前架构决策见 [为什么 SwarmDrop 暂不迁移 Iroh](../why-libp2p-not-iroh.md)。
> 除「n0-future 替换」等可与迁移解耦的建议外，其余内容仅在未来重新评估 Iroh 时适用。
> 带 ⚠️ 标注的条目是**建议/待评审**，不是从源码验证出的事实。
