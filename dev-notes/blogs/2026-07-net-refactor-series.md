...
> 抽成可依赖倒置的独立 crate 的完整记录。拆成 5 个主题系列，每篇聚焦一个内聚知识点。
...
`transfer/` 下的旧文（`end-to-end-encryption.md`、`transfer-protocol-design.md` 等）
描述的是重构**前**的架构（XChaCha20 加密、旧 wire、单 crate）。本系列的
`transfer-architecture/` 反映重构后的形态，相关篇会注明取代关系。
