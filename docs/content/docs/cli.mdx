...
环境变量排在前面是刻意的：命令行端常跑在脚本与服务单元里，那些地方设一个变量比维护一份
配置自然得多。两者同时存在时 `config list` 会明说哪个在生效、以及被压住的那个是什么。

有常驻节点在跑时改落点**当场生效**，不必重启它；已经收下的文件留在原处不动。
...
记下来的是**你做了什么**（加了哪几条、撤了哪几条），不是合并后的清单——所以将来版本更新
换掉内置地址时，你照样拿得到新的。
