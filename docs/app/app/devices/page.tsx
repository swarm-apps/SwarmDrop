...
//
// ⚠️ **配对在 DOM 里是第二块**（`DevicesSection` 渲染网格与配对两个 `SectionShell`，
// 活跃传输是页面这一层的第二个兄弟）。它排在活跃传输之前而不是最后，是因为它与设备网格
// 属于同一件事——「这个列表里有谁、怎么让它多一个」；「按使用频率排」管的是块与块的相对
// 位置，而收起态下它只占一行标题，不与活跃传输争版面。
// 写 e2e 选择器时以此为准，别按注释里的序号猜。
//
