...
// ## 「使用文档」为什么不是一整行
//
// 它此前独占一行、图标加文字，视觉重量与上面三条主路由持平——而它是**离开应用区**的链接。
// 一个走出 `/app` 的入口不该和「设备」「收件箱」看起来一样重。降级之后它仍然在场——
// 知识库那条「从 chrome 上拿掉的东西必须重新出现」说的是**拿掉**，这里是降级。
//
// ⚠️ 这里此前还跟着一句「并且离开 `/app` 会卸载节点单例、中断正在进行的传输」，**是错的**
// （2026-08-06 核实）。节点运行时是模块级单例，寿命跟着页面而不是 React 树，客户端跳出
// `/app` 只卸载 DOM。完整判据与实测写在 `app-nav.tsx` 的 `BrandMark` 头注释里。
// 那句话的代价是真的：它一度让「点 logo 回官网」这个入口被判为不可做。
//
// ## 三档断点下的形态
//
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" side="top" className="w-48">
        ...
        {/* 出应用区的链接：右侧那枚箭头是它与上面几项的唯一区别——上面的都在原地改环境，
            点它会走到 `/app` 之外。（传输不受影响，见文件头那条更正。） */}
