...
export function WebErrorCard({ error, className = "" }: { error: WebError; className?: string }) {
  const { t } = useLingui();
  return (
    <div
      role="alert"
    ...
    >
      <p className="font-medium text-red-900 dark:text-red-200">{t(WEB_ERROR_KIND_LABEL[error.kind])}</p>
