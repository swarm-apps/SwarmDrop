...
// | 层不透明度 | 1 | **暗 1（与桌面持平）· 亮 0.34**。分叉是对比度定的，不是审美——推导与实测表见 `global.css` 的两处变量注释 |
// | 遮罩 | 无 | 径向 `mask-image`，**按主题两档**：着色器把光带钉在画面中线，1240px 限宽 + 32px 区块间距会让它从缝隙里原样透出来。暗色中心留 45%（玻璃要有东西可折），亮色中心全挖空 |
//
// ⚠️ 这张表此前写着「暗 0.7 / 亮 0.48」，而代码里是 0.34——**注释漂了一个值**。改这两个数时
// 连同 `global.css` 的注释与 DESIGN.md 的表一起改，三处都写着推导，漂一处就会误导下一个人。
//
