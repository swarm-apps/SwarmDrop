...
//
// **不透明度与遮罩已不在这张表里**（2026-08-06 起两端同参）：暗 1 / 亮 0.34，配一道按主题
// 两档的径向遮罩。它们曾经是 Web 端独有的压制，而那在两个方向上都错了——Web 端暗色的 0.7
// 把极光压到了玻璃根本无法工作的水平，桌面端则不带遮罩、任由着色器的中线光带成为整屏最亮的
// 东西。推导与实测表见 `global.css` 的两处变量注释与桌面 `src/index.css` 的同名变量。
//
// ⚠️ 这张表此前写着「暗 0.7 / 亮 0.48」，而代码里是 0.34——**注释漂了一个值**。这几个数现在
// 写在三处（两端 CSS + DESIGN.md），改一个数要三处一起改。
//
